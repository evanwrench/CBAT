...
bool AUTORQ = false;
bool AUTOINSDEL = false;
const char* QUERY_TEST_TYPE;
...
#ifdef USE_RCU
    #include "../ds/arbel_int_bst_lf/citrus/urcu_impl.h"
    #define __RCU_INIT_THREAD urcu::registerThread(tid);
    #define __RCU_DEINIT_THREAD urcu::unregisterThread();
    #define __RCU_INIT_ALL urcu::init(std::max(TOTAL_THREADS, PREFILL_THREADS));
    #define __RCU_DEINIT_ALL urcu::deinit(std::max(TOTAL_THREADS, PREFILL_THREADS));
#else
...
#define THREAD_MEASURED_PRE \
    ...
    int rq_cnt = 0; \
    DURATION_START(tid);
// papi_create_eventset(tid); \ g->dsAdapter
// papi_start_counters(tid); \ gstats
...
struct globals_t {
    ...
    std::atomic<long long int> sortedCounter;
...
//    PAD; // not needed because of padding at the end of rngs
    ...
    uint64_t rq_timestamps[MAX_THREADS_POW2 * PREFETCH_SIZE_WORDS];
    uint64_t up_timestamps[MAX_THREADS_POW2 * PREFETCH_SIZE_WORDS];
    volatile bool start;
    PAD;
    volatile bool done;
    PAD;
    ...
    , PREFILL_INTERVAL_MILLIS(200)
    {
        ...
        prefillSize = 0;
        PAD;
        sortedCounter = MAX_THREADS_POW2*1000;
    }
...
void thread_timed(GlobalsT * g, int __tid) {
    THREAD_MEASURED_PRE;
    int tid = __tid;
    test_type lastkey = ((MAX_THREADS_POW2 - __tid) - 1)*100 + 1;
    while (!g->done) {
        ++cnt;
        ...
        if (op < INS_FRAC) {
            ...
            if (SORTED_WORKLOAD) {
                if (lastkey % 100 == 0) {
                    key = (g->sortedCounter.fetch_add(100) % MAXKEY) + 1;
                } else {
                    key = lastkey + 1;
                }
                lastkey = key;
            ...
            g->up_timestamps[tid*PREFETCH_SIZE_WORDS] += (end_time - start_time);
            ...
            uint64_t end_time = GETTIMESTAMP();
            g->rq_timestamps[tid*PREFETCH_SIZE_WORDS] += (end_time - start_time);
            GSTATS_ADD(tid, num_rq, 1);
        } else if (op < INS_FRAC+DEL_FRAC+RQ+RANK) {
            g->dsAdapter->getRank(tid, key, rqResultKeys, (VALUE_TYPE *) rqResultValues);
            GSTATS_ADD(tid, num_rk, 1);
        } else if (op < INS_FRAC+DEL_FRAC+RQ+RANK+SELECT) {
            const double expectedFullness = (INS_FRAC+DEL_FRAC ? INS_FRAC / (double)(INS_FRAC+DEL_FRAC) : 0.5); // percent full in expectation
            int rank = (g->rngs[tid].next() % MAXKEY) * expectedFullness;
            g->dsAdapter->selectItem(tid, rank);
            GSTATS_ADD(tid, num_sel, 1);
        } else {
...
void printOutput(GlobalsT * g) {
...
#ifdef USE_GSTATS
    {
        ...
        const long long totalRanks = GSTATS_GET_STAT_METRICS(num_rk, TOTAL)[0].sum;
        const long long totalSelects = GSTATS_GET_STAT_METRICS(num_sel, TOTAL)[0].sum;
        const long long totalQueries = totalSearches + totalRQs + totalRanks + totalSelects;
        ...
        uint64_t tot_up = 0;
        for (int i = 0; i < MAX_THREADS_POW2; i++) {
            tot_rq += g->rq_timestamps[i * PREFETCH_SIZE_WORDS];
            tot_up += g->up_timestamps[i * PREFETCH_SIZE_WORDS];
        }
        float seconds_for_rqs = (1.*tot_rq)/(tot_rq+tot_up)*SECONDS_TO_RUN;
        float seconds_for_ups = SECONDS_TO_RUN - seconds_for_rqs;

        COUTATOMIC(std::endl);
        COUTATOMIC("total_find="<<totalSearches<<std::endl);
        COUTATOMIC("total_rq="<<totalRQs<<std::endl);
        COUTATOMIC("total_ranks="<<totalRanks<<std::endl);
        COUTATOMIC("total_selects="<<totalSelects<<std::endl);
        ...
        COUTATOMIC("query_seconds="<<seconds_for_rqs<<std::endl);
        COUTATOMIC("update_seconds="<<seconds_for_ups<<std::endl);
        COUTATOMIC("avg_seconds_for_query="<<(seconds_for_rqs/totalQueries)<<std::endl);
        COUTATOMIC("avg_seconds_for_update="<<(seconds_for_ups/totalUpdates)<<std::endl);
        ...
        COUTATOMIC("total ranks                   : "<<totalRanks<<std::endl);
        COUTATOMIC("total selects                 : "<<totalSelects<<std::endl);
...
int main(int argc, char** argv) {
    ...
    for (int i=1;i<argc;++i) {
        if (strcmp(argv[i], "-i") == 0) {
            ...
            RQ = atof(argv[++i]);
            QUERY_TEST_TYPE = "-rq";
        } else if (strcmp(argv[i], "-rk") == 0) {
            RANK = atof(argv[++i]);
            QUERY_TEST_TYPE = "-rk";
        } else if (strcmp(argv[i], "-sel") == 0) {
            SELECT = atof(argv[++i]);
            QUERY_TEST_TYPE = "-sel";
        } else if (strcmp(argv[i], "-autorq") == 0) {
            AUTORQ = true;
        } else if (strcmp(argv[i], "-autoinsdel") == 0) {
            AUTOINSDEL = true;
        ...
        } else if (strcmp(argv[i], "-prefillfrac") == 0) {
            PREFILL_FRAC = atof(argv[++i]);
        } else if (strcmp(argv[i], "-dist-zipf") == 0) {
            ZIPF_PARAM = atof(argv[++i]);
            if (ZIPF_PARAM > 0.00001) {
        ...
        } else if (strcmp(argv[i], "-sorted-workload") == 0) {
            SORTED_WORKLOAD = true;
    ...
    if (AUTOINSDEL) {
        INS_FRAC = 50.0 - (RQ + RANK + SELECT)/2;
        DEL_FRAC = 50.0 - (RQ + RANK + SELECT)/2;
    }
    if (AUTORQ) {
        RQ = 50.0 - (INS_FRAC + DEL_FRAC + RANK + SELECT)/2;
    }
    if (PREFILL_FRAC > 0) {
        DESIRED_PREFILL_SIZE = MAXKEY * PREFILL_FRAC;
    }
