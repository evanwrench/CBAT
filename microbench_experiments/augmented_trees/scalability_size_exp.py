...
def plot_func(filename, column_filters, data, series_name, x_name, y_name, exp_dict=None):
    ...
    ax = table.plot(kind='line', logx=True, logy=True, color=['tab:blue', 'tab:orange', 'tab:cyan', 'tab:green', 'tab:purple', 'tab:red'], style=['o-', 's-', 'P-', '^-', 'v-', 'D-'], ylabel='Throughput') # ['o-', 's-', 'v-', '^-', 'D-', '<-']
    ax.set_xlabel("Max Key", fontsize=14)
    ax.set_ylabel("Throughput", fontsize=14)
    ax.set_ylim(bottom=10000)
    ax.tick_params(axis='both', which='major', labelsize=12)
    ax.tick_params(axis='both', which='minor', labelsize=12)
    handles, labels = ax.get_legend_handles_labels()
    order = [2, 3, 0, 1, 5, 4] # order = [2, 3, 4, 0, 1, 5]
    reordered_handles = [handles[i] for i in order]
    reordered_labels = [labels[i] for i in order]
    ax.legend(reordered_handles, reordered_labels, fontsize=11, loc='lower left')
...
def define_experiment(exp_dict, args):
    ...
    set_dir_data     (exp_dict, os.getcwd() + '/data_scalability_size')

    add_run_param    (exp_dict, 'TOTAL_THREADS', [min(120, int(sys.argv[1]))])
    add_run_param    (exp_dict, '__trials', [1, 2, 3, 4, 5])
    add_run_param    (exp_dict, 'DS_TYPENAME', ['brown_ext_chromatic_augment_lf', 'brown_ext_chromatic_delegateSingle_lf', 'wei_ext_vcas_bst_lf', 'arbel_int_bst_lf', 'blelloch_ext_btree_lf', 'ellen_augmented_ext_bst_lf'])
    add_run_param    (exp_dict, 'MAXKEY', [100000, 400000, 3000000, 10000000]) 
    add_run_param    (exp_dict, 'RQSIZE', [50000])
    add_run_param    (exp_dict, 'ZIPF_PARAM', ['0', '0.95'])
    add_run_param    (exp_dict, 'INS_DEL_FRAC', ['2.5 2.5', '25.0 25.0'])
    ...
    add_plot_set(
            exp_dict
          , name='scal_size_threads=zipf={ZIPF_PARAM}_insdel={INS_DEL_FRAC}.pdf'
          , title='Max Key vs Throughput'
          , varying_cols_list=['ZIPF_PARAM', 'INS_DEL_FRAC']
          , series='DS_TYPENAME'
          , x_axis='MAXKEY'
