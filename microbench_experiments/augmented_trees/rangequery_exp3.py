...
def plot_func(filename, column_filters, data, series_name, x_name, y_name, exp_dict=None):
    ...
    ax.set_ylabel("Range Query Latency (Seconds)", fontsize=14)
...
def define_experiment(exp_dict, args):
    ...
    set_dir_data     (exp_dict, os.getcwd() + '/data3_rangequeryTime')
    ...
    add_data_field   (exp_dict, 'avg_seconds_for_query', coltype='REAL')
    ...
    add_plot_set(
            exp_dict
          , name='query_time.pdf'
          , title='Range Query Size vs Average Range Query Time'
          , varying_cols_list=[]
          , series='DS_TYPENAME'
          , x_axis='RQSIZE'
          , y_axis='avg_seconds_for_query'
          , plot_type=plot_func
    )
