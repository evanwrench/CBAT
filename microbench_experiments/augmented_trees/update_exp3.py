...
def plot_func(filename, column_filters, data, series_name, x_name, y_name, exp_dict=None):
    ...
    ax.set_ylabel("Update Latency (Seconds)", fontsize=14)
...
def define_experiment(exp_dict, args):
    ...
    set_dir_data     (exp_dict, os.getcwd() + '/data3_updateTime')
    ...
    add_data_field   (exp_dict, 'avg_seconds_for_update', coltype='REAL')
    ...
    add_plot_set(
            exp_dict
          , name='update_time.pdf'
          , title='Range Query Size vs Average Update Time'
          , varying_cols_list=[]
          , series='DS_TYPENAME'
          , x_axis='RQSIZE'
          , y_axis='avg_seconds_for_update'
          , plot_type=plot_func
    )
