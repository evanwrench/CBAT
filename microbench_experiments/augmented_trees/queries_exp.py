...
def plot_func(filename, column_filters, data, series_name, x_name, y_name, exp_dict=None):
    ...
    ax = table.plot(kind='line', style=['o-', 's-', '^-'], ylabel='Throughput (Mop/s)')
...
def define_experiment(exp_dict, args):
    ...
    set_dir_data     (exp_dict, os.getcwd() + '/data_queries')
    ...
    add_run_param    (exp_dict, 'QUERY_TEST_TYPE', ['-sel', '-rk', '-rq'])
    ...
    set_cmd_run      (exp_dict, 'LD_PRELOAD=../lib/libmimalloc.so ./bin/brown_ext_chromatic_delegateSingle_lf.debra -nwork {TOTAL_THREADS} -nprefill 30 -insdel {INS_DEL_FRAC} {QUERY_TEST_TYPE} 90 -rqsize {RQSIZE} -dist-zipf {ZIPF_PARAM} -k {MAXKEY} -t 3000')
