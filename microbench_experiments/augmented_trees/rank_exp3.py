...
def plot_func(filename, column_filters, data, series_name, x_name, y_name, exp_dict=None):
    ...
    ax = table.plot(kind='line', logx=True, logy=True, color=['tab:blue', 'tab:orange', 'tab:green', 'tab:purple', 'tab:red'], style=['o-', 's-', '^-', 'v-', 'D-'], ylabel='Throughput')
...
def define_experiment(exp_dict, args):
    ...
    set_dir_data     (exp_dict, os.getcwd() + '/data_rank3')
    ...
    set_cmd_run      (exp_dict, 'LD_PRELOAD=../lib/libmimalloc.so ./bin/{DS_TYPENAME}.debra -nwork {TOTAL_THREADS} -nprefill 30 -autoinsdel -rk {RANK} -dist-zipf {ZIPF_PARAM} -k {MAXKEY} -t 3000')
    
    # Rank
    add_plot_set(
            exp_dict
          , name='rank3_maxkey={MAXKEY}.pdf'
