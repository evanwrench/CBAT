...
def plot_func(filename, column_filters, data, series_name, x_name, y_name, exp_dict=None):
    ...
    table_base = table_base / 1000000
    table = table_base.rename(columns=series_name_map)
    ax = table.plot(kind='line', color=['tab:cyan', 'tab:pink', 'tab:green', 'tab:purple'], style=['P-', 'X-', '^-', 'v-'], ylabel='Throughput (Mop/s)')
    ax.set_xlabel("Number of Threads", fontsize=14)
    ax.set_ylabel("Throughput (Mop/s)", fontsize=14)
...
def define_experiment(exp_dict, args):
    ...
    set_dir_data     (exp_dict, os.getcwd() + '/data_balance')
    ...
    add_run_param    (exp_dict, 'DS_TYPENAME', ['brown_ext_chromatic_augment_lf', 'brown_ext_chromatic_delegateDouble_lf', 'brown_ext_chromatic_delegateSingle_lf', 'ellen_augmented_ext_bst_lf'])
    add_run_param    (exp_dict, 'MAXKEY', [10000000])
    add_run_param    (exp_dict, 'RQSIZE', [50000])
    add_run_param    (exp_dict, 'ZIPF_PARAM', ['0'])
    add_run_param    (exp_dict, 'INS_DEL_FRAC', ['100.0 0.0'])
    add_data_field   (exp_dict, 'total_throughput', coltype='INTEGER')

    set_cmd_run      (exp_dict, 'LD_PRELOAD=../lib/libmimalloc.so ./bin/{DS_TYPENAME}.debra -nwork {TOTAL_THREADS} -nprefill 0 -insdel {INS_DEL_FRAC} -rq 0 -dist-zipf {ZIPF_PARAM} -rqsize {RQSIZE} -k {MAXKEY} -sorted-workload -t 3000')
