...
def plot_func(filename, column_filters, data, series_name, x_name, y_name, exp_dict=None):
    ...
    ax = table.plot(kind='line', logy=True, color=['tab:blue', 'tab:orange', 'tab:green', 'tab:purple', 'tab:red'], style=['o-', 's-', '^-', 'v-', 'D-'], ylabel='Throughput')
...
def define_experiment(exp_dict, args):
    ...
    set_dir_data     (exp_dict, os.getcwd() + '/data_scalability_thread')
    ...
    add_plot_set(
            exp_dict
          , name='scal_thread_maxkey=10000000_insdel={INS_DEL_FRAC}.pdf'
