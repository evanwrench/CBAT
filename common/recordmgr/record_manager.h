...
public:
    ...
    template <typename T>
    record_manager_single_type_super<Reclaim, Alloc, Pool, T>* get(T * const recordType) {
...
class RecordManagerSet<Reclaim, Alloc, Pool, First, Rest...> : RecordManagerSet<Reclaim, Alloc, Pool, Rest...> {
    PAD;
    record_manager_single_type<Reclaim, Alloc, Pool, First, Rest...> * const mgr;
	PAD;
public:
    RecordManagerSet(const int numProcesses, RecoveryMgr<void *> * const _recoveryMgr)
        : RecordManagerSet<Reclaim, Alloc, Pool, Rest...>(numProcesses, _recoveryMgr)
        , mgr(new record_manager_single_type<Reclaim, Alloc, Pool, First, Rest...>(numProcesses, _recoveryMgr, this))
        {
    ...
    template<typename T>
    inline record_manager_single_type_super<Reclaim, Alloc, Pool, T>* get(T * const recordType) {
        if (typeid(First) == typeid(T)) {
            //cout<<"MATCH: typeid(First)="<<typeid(First).name()<<" typeid(T)="<<typeid(T).name()<<std::endl;
            return (record_manager_single_type<Reclaim, Alloc, Pool, T, Rest...> *) mgr;
        } else {
    ...
    inline void startOp(const int tid, const bool callForEach, const bool readOnly = false) {
        if (callForEach) {
            ...
            ((record_manager_single_type<Reclaim, Alloc, Pool, First, Rest...>*) get((First *) NULL))->template startOp<First, Rest...>(tid, reclaimers, 1+sizeof...(Rest), readOnly);
    ...
    inline void debugGCSingleThreaded() {
        ...
        ((record_manager_single_type<Reclaim, Alloc, Pool, First, Rest...>*) get((First *) NULL))->template debugGCSingleThreaded<First, Rest...>(reclaimers, 1+sizeof...(Rest));
        __sync_synchronize(); // memory barrier needed (only) for epoch based schemes at the moment...
    }
};
...
//        endOp(tid);
    ...
    template <typename T>
    inline record_manager_single_type<Reclaim, Alloc, Pool, T> * get(T * const recordType) {
