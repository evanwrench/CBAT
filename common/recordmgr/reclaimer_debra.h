...
#include "reclaimer_interface.h"
    ...
    #include "../ds/ellen_augmented_ext_bst_lf/ellen_impl.h"
...
template <typename T = void, class Alloc = allocator_interface<T>, class Pool = pool_interface<T>, typename ...RestT>
class reclaimer_debra : public reclaimer_interface<T, Alloc, Pool, RestT...> {
protected:
...
#define NUMBER_OF_EPOCH_BAGS 9 // 9 for range query support
#define NUMBER_OF_ALWAYS_EMPTY_EPOCH_BAGS 3 // 3 for range query support
...
//    PAD; // not needed after superclass layout
    ...
    PAD;
    #ifdef DEBRA_NODE_RETIRE
        RecordManagerSet<reclaimer_debra<>,allocator_new<>,pool_none<>,T,RestT...> * rmset;//T, Alloc, Pool, RestT...
        PAD;
    #endif
...
public:
    ...
    struct rebind4 {
        typedef reclaimer_debra<_Tp1, _Tp2, _Tp3, _Tp4...> other;
    };
...
#ifdef DEBRA_NODE_RETIRE
        if (std::is_base_of<Node_Super<test_type, VALUE_TYPE>, T>::value) {
            for (blockbag_iterator<T> it = freeable->begin(); it != freeable->end(); it++) {
                Node_Super<test_type, VALUE_TYPE>* item = (Node_Super<test_type, VALUE_TYPE>*) (*it);
                rmset->get((Version<test_type>*) NULL)->retire(tid, (Version<test_type>*) item->version.load(memory_order_relaxed));
            }
        } else if (std::is_base_of<node_t<test_type, VALUE_TYPE>, T>::value) {
            for (blockbag_iterator<T> it = freeable->begin(); it != freeable->end(); it++) {
                node_t<test_type, VALUE_TYPE>* item = (node_t<test_type, VALUE_TYPE>*) (*it);
                rmset->get((version_t<test_type>*) NULL)->retire(tid, item->version);
            }
        }
#endif
#ifdef GSTATS_HANDLE_STATS
...
#   endif
    ...
    public:
        inline void rotateAllEpochBags(const int tid, void * const * const reclaimers, const int i) {
            ...
            ((reclaimer_debra<First, classAlloc, classPool, Rest...> * const) reclaimers[i])->rotateEpochBags(tid);
...
#   endif
    ...
    // void printTypes() {
    //     COUTATOMIC(typeid(F).name()<<std::endl);
    //     printTypes<I, L...>();
    // }
    reclaimer_debra(const int numProcesses, Pool *_pool, debugInfo * const _debug, RecoveryMgr<void *> * const _recoveryMgr = NULL, RecordManagerSet<reclaimer_debra<>,allocator_new<>,pool_none<>,T,RestT...> * rm = nullptr) //T, Alloc, Pool, RestT...
            : reclaimer_interface<T, Alloc, Pool, RestT...>(numProcesses, _pool, _debug, _recoveryMgr) {
