...
template <class Reclaim, class Alloc, class Pool, typename Record>
class record_manager_single_type_super {
protected:
...
public:
    ...
    record_manager_single_type_super() {
        VERBOSE DEBUG COUTATOMIC("constructor record_manager_single_type"<<std::endl);
    }

    virtual void initThread(const int tid) {}
    virtual void deinitThread(const int tid) {}
    virtual inline void clearCounters() {}
    inline static bool shouldHelp() { // FOR DEBUGGING PURPOSES
        return Reclaim::shouldHelp();
    }
    virtual inline bool isProtected(const int tid, record_pointer obj) {return false;}
    // for hazard pointers (and reference counting)
    virtual inline bool protect(const int tid, record_pointer obj, CallbackType notRetiredCallback, CallbackArg callbackArg, bool hintMemoryBarrier = true) {return false;}
    virtual inline void unprotect(const int tid, record_pointer obj) {}
    // warning: qProtect must be reentrant and lock-free (=== async-signal-safe)
    virtual inline bool qProtect(const int tid, record_pointer obj, CallbackType notRetiredCallback, CallbackArg callbackArg, bool hintMemoryBarrier = true) {return false;}
    virtual inline void qUnprotectAll(const int tid) {}
    virtual inline bool isQProtected(const int tid, record_pointer obj) {return false;}
    ...
    virtual inline bool isQuiescent(const int tid) {return false;}
    // for epoch based reclamation
    virtual inline void endOp(const int tid) {}
    ...
    virtual inline void retire(const int tid, record_pointer p) {};
    // for all schemes
    virtual inline record_pointer allocate(const int tid) {return nullptr;}
    virtual inline void deallocate(const int tid, record_pointer p) {}
    virtual void printStatus(void) {}
};

// maybe Record should be a size
template <class Reclaim, class Alloc, class Pool, typename Record, typename... RestRecords>
class record_manager_single_type: public record_manager_single_type_super<Reclaim, Alloc, Pool, Record> {
protected:
    typedef Record* record_pointer;

    typedef typename Alloc::template    rebind<Record>::other              classAlloc;
    typedef typename Pool::template     rebind2<Record, classAlloc>::other classPool;
    typedef typename Reclaim::template  rebind4<Record, classAlloc, classPool, RestRecords...>::other classReclaim;

    RecordManagerSet<Reclaim,Alloc,Pool,Record,RestRecords...> * rmset;
...
public:
    ...
    record_manager_single_type(const int numProcesses, RecoveryMgr<void *> * const _recoveryMgr, RecordManagerSet<Reclaim,Alloc,Pool,Record,RestRecords...> * rm)
...
#ifdef DEBRA_NODE_RETIRE
        reclaim = new classReclaim(numProcesses, pool, &debugInfoRecord, recoveryMgr, rm);
#else
    ...
    inline void retire(const int tid, Record* p) {
