...
uintptr_t Chromatic<K,V,Compare,MasterRecordMgr>::getVersion(Node<K,V>* x) {
    if ((Version<K>*) x->version.load(memory_order_relaxed) == NULL) {
        refresh(x, nullptr, nullptr);
    }
    return x->version.load(memory_order_relaxed);
}

template<class K, class V, class Compare, class MasterRecordMgr>
PropagateStatus* Chromatic<K,V,Compare,MasterRecordMgr>::refresh(Node<K, V>* x, PropagateStatus* ps, vector<Version<K>*> *toRetire) {
    uintptr_t oldVer = x->version.load(memory_order_relaxed); // TODO: refresh while value isn't changed?
    if(toRetire == nullptr && oldVer != (uintptr_t) nullptr) return nullptr;
    ...
    // leftver = (Version<K>*) getVersion(left1);
    // right1 = (Node<K,V>*) x->right.load(memory_order_relaxed);
    // rightver = right1 ? (Version<K>*) getVersion(right1) : NULL;
    // bool fls = false;
    // bool tru = true;

    // if (x->mx.compare_exchange_strong(fls, true)) {
    //     if ((Version<K>*) x->lver.load(memory_order_relaxed) == leftver) {
    //         lsum = x->lsum.load(memory_order_relaxed);
    //     } else {
    //         lsum = leftver->sum;
    //         x->lsum.store(lsum);
    //         x->lver.store((uintptr_t) leftver);
    //     }
    //     x->mx.compare_exchange_strong(tru, false);
    // } else {
    //     lsum = leftver->sum;
    // }
    newVer = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), x->key, rightver ? leftver->sum + rightver->sum : leftver->sum, leftver, rightver, ps);
...
void Chromatic<K,V,Compare,MasterRecordMgr>::propagate(int tid, K key, stack<Node<K,V>*> stack) {
    ...
    // bool goDown = false;
    // Node<K,V>* c = root;
    // while (((Node<K,V>*)c->left.load(memory_order_relaxed))) { //  && !set.count(c)
    //     stack.push(c);
    ...
    //         goDown = true;
    //     } else if (goDown) {
    //         Node<K,V>* n = cmp(key, r->key) || r->key == NO_KEY ? (Node<K,V>*) r->left.load(memory_order_relaxed) : (Node<K,V>*) r->right.load(memory_order_relaxed);
    //         if (set.count(n) || !((Node<K,V>*)n->left.load(memory_order_relaxed))) {
    //             goDown = false;
    //         } else {
    //             stack.push(n);
    //         }
    //     } else {
    //         PropagateStatus* curps = refresh(r, ps, toRetire);
    //         if (curps != ps) {
    //             curps = refresh(r, ps, toRetire);
    //             if (curps != ps) {
    //                 ps->delegatee = curps;
    //                 break;
    //             }
    ...
    while (!set.count(root)) {
        ...
        if (curps != ps) {
            curps = refresh(r, ps, &toRetire);
            if (curps != ps && !r->marked.load(memory_order_relaxed)) {
                ps->delegatee = curps;
                break;
            }
        }
