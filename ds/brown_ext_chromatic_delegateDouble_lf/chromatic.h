...
private:
    ...
    PropagateStatus* refresh(Node<K, V>*, PropagateStatus*, vector<Version<K>*>*);
...
public:
    ...
    void dfsDeallocateBottomUp(Node<K,V> * const u, set<void*>& seen, int *numNodes) {
        ...
        recordmgr->deallocate(0, (Version<K>*) u->version.load(memory_order_relaxed));
    ...
    ~Chromatic() {
        ...
        if (size > 0) {
            ...
            ofstream tracefile;
            tracefile.open("traces/delegateErrorFixed.txt", ios::app);
    ...
    const int doRangeQuery(const int tid, const K& lo, const K& hi, const Version<K>* curr);
