...
public:
    ...
    int bst_nonlinearizable_rangequery(const int tid, skey_t lo, skey_t hi);
    int do_bst_nonlinearizable_rangequery(const int tid, skey_t lo, skey_t hi, node_t<skey_t, sval_t>* curr);
...
template <typename skey_t, typename sval_t, class RecMgr>
int ellen<skey_t, sval_t, RecMgr>::bst_nonlinearizable_rangequery(const int tid, skey_t lo, skey_t hi) {
    auto guard = recmgr->getGuard(tid, true);
    return do_bst_nonlinearizable_rangequery(tid, lo, hi, root);
}

template <typename skey_t, typename sval_t, class RecMgr>
int ellen<skey_t, sval_t, RecMgr>::do_bst_nonlinearizable_rangequery(const int tid, skey_t lo, skey_t hi, node_t<skey_t, sval_t>* curr) {
    if (curr->left == NULL) {
        return (curr->key == KEY_MAX || curr->key < lo || curr->key > hi) ? 0 : 1;
    }
    if (curr->key < lo) {
        return do_bst_nonlinearizable_rangequery(tid, lo, hi, curr->right);
    }
    if (curr->key > hi) {
        return do_bst_nonlinearizable_rangequery(tid, lo, hi, curr->left);
    }
    return do_bst_nonlinearizable_rangequery(tid, lo, hi, curr->left) + do_bst_nonlinearizable_rangequery(tid, lo, hi, curr->right);
}
