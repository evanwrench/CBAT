...
template<class K, class V, class Compare, class RecManager>
int vcas_bst_ns::vcas_bst<K,V,Compare,RecManager>::getRank(const int tid, const K& key, K * const resultKeys, V * const resultValues) {
    bool found = false;
    int res = rangeQuery(tid, 0, key, resultKeys, resultValues, found);
    return res; //found ? res : -1;
}

template<class K, class V, class Compare, class RecManager>
int vcas_bst_ns::vcas_bst<K,V,Compare,RecManager>::rangeQuery(const int tid, const K& lo, const K& hi, K * const resultKeys, V * const resultValues, bool &found) {
