...
#ifndef RECORD_MANAGER_VCAS_H
#define	RECORD_MANAGER_VCAS_H
...
public:
    RecordManagerSet(const int numProcesses, RecoverVcas::RecoveryMgr<void *> * const _recoveryMgr) {}
    template <typename T>
...
public:
    ...
    RecoverVcas::RecoveryMgr<SelfType> * const recoveryMgr;
    volatile char padding1[PREFETCH_SIZE_BYTES];
    
    record_manager(const int numProcesses, const int _neutralizeSignal)
            : NUM_PROCESSES(numProcesses)
            , recoveryMgr(new RecoverVcas::RecoveryMgr<SelfType>(numProcesses, _neutralizeSignal, this))
    {
        rmset = new RecordManagerSet<Reclaim, Alloc, Pool, RecordTypesFirst, RecordTypesRest...>(numProcesses, (RecoverVcas::RecoveryMgr<void *> *) recoveryMgr);
    }
    ~record_manager() {
        delete recoveryMgr;
        delete rmset;
