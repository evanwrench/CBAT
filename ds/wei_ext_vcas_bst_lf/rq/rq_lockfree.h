...
public:
    ...
    inline void traversal_start(const int tid) {
        ...
        threadData[tid].rq_lin_time = takeSnapshot();      // linearize rq here!
...
private:
    ...
    inline int __traversal_try_add(const int tid, NodeType * const node, K * const outputKeys, V * const outputValues, const K& lo, const K& hi, bool foundDuringTraversal) {
        ...
        if (!ds->isLogicallyInserted(tid, node)) return 0; // node inserted after the range query
        // fact: node has been logically inserted

        long long itime = node->itime;
        if (itime != TIMESTAMP_NOT_SET & node->itime >= threadData[tid].rq_lin_time) return 0; // node inserted after the range query
