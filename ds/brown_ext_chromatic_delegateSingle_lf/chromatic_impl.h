...
#define initializeVersion(_tid, _newver, _key, _sum, _left, _right, _status) \
(_newver); \
{ \
    (_newver)->key = (_key); \
    (_newver)->sum = (_sum); \
    ...
    (_newver)->status.store((uintptr_t) (_status), memory_order_relaxed); \
}
...
{ \
    ...
    if((_left) == nullptr) { \
        ...
        initializeVersion(_tid, _vernode, _key, 1, nullptr, nullptr, nullptr); \
        (_newnode)->version.store((uintptr_t) (_vernode), memory_order_relaxed); \
    } \
    else (_newnode)->version.store((uintptr_t) nullptr, memory_order_relaxed); \
}
#define initializeStatus(_tid, _newstatus, _delegatee) \
(_newstatus); \
{ \
    (_newstatus)->delegatee = _delegatee; \
}
...
Chromatic<K,V,Compare,MasterRecordMgr>::Chromatic(const K& _NO_KEY,
    ...
    allocatedStatuses = new PropagateStatus*[numProcesses*PREFETCH_SIZE_WORDS];
    traces = new vector<tuple<int, uint64_t, int, bool, K, K, int>>[numProcesses];
    ...
    initializeVersion(tid, rootleftver, NO_KEY, 0, NULL, NULL, nullptr);
    Version<K>* rootver = allocateVersion(tid);
    initializeVersion(tid, rootver, NO_KEY, 0, rootleftver, NULL, nullptr);
    ...
    for (;;) {
        // CHECKPOINT_AND_RUN_QUERY(tid) {
            ...
            if (key == l->key) {
                ...
                // TRACE COUTATOMICTSTID("find("<<key<<") found key"<<endl);
                TRACE traces[tid].push_back(tuple<int, uint64_t, int, bool, K, K, int>(tid, GETTIMESTAMP(), 2, 1, key, -1, 1));
            } else {
                // TRACE COUTATOMICTSTID("find("<<key<<") did not find key"<<endl);
                TRACE traces[tid].push_back(tuple<int, uint64_t, int, bool, K, K, int>(tid, GETTIMESTAMP(), 2, 1, key, -1, 0));
            }
            return result; // success
        // }
...
template<class K, class V, class Compare, class MasterRecordMgr>
const K Chromatic<K,V,Compare,MasterRecordMgr>::selectItem(const int tid, int rank) {
    auto guard = recordmgr->getGuard(tid, true);
    return doSelectItem(tid, rank, (Version<K>*) root->version.load(memory_order_relaxed));
}

template<class K, class V, class Compare, class MasterRecordMgr>
const K Chromatic<K,V,Compare,MasterRecordMgr>::doSelectItem(const int tid, int rank, Version<K>* curr) {
    if (((Version<K>*) curr->left.load(memory_order_relaxed)) == NULL) {
        if (rank == 1) {
            return curr->key;
        } else {
            return NO_KEY; // Key not found
        }
    }
    Version<K>* currLeft = (Version<K>*)curr->left.load(memory_order_relaxed);
    Version<K>* currRight = (Version<K>*)curr->left.load(memory_order_relaxed);
    if (rank <= currLeft->sum) {
        return doSelectItem(tid, rank, currLeft);
    }
    return doSelectItem(tid, rank-currLeft->sum, currRight);
}
...
const int Chromatic<K,V,Compare,MasterRecordMgr>::rangeQuery(const int tid, const K& lo, const K& hi) {
    ...
    TRACE traces[tid].push_back(tuple<int, uint64_t, int, bool, K, K, int>(tid, GETTIMESTAMP(), 3, 0, lo, hi, -1));
    auto guard = recordmgr->getGuard(tid, true);
    ...
    TRACE traces[tid].push_back(tuple<int, uint64_t, int, bool, K, K, int>(tid, GETTIMESTAMP(), 3, 1, lo, hi, res));
    return res;
}

template<class K, class V, class Compare, class MasterRecordMgr>
const int Chromatic<K,V,Compare,MasterRecordMgr>::doRangeQuery(const int tid, const K& lo, const K& hi, Version<K>* curr) {
...
//            assert(!recordmgr->isProtected(tid, p));
    ...
    if (key == l->key) {
        if (onlyIfAbsent) {
            assert(recordmgr->isProtected(tid, l));
            *result = l->value;
            // TRACE COUTATOMICTSTID("insert("<<key<<") already in tree"<<endl);
            return true; // success
        }
        ...
        *result = l->value;
        ...
        initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), key, 1 /* sum */, NULL, NULL, nullptr);
        ...
        /*Node<K,V> *newLeaf =*/
        initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), key, 1, NULL, NULL, nullptr);
        initializeNodeWithVersion(tid, GET_ALLOCATED_NODE_PTR(tid, 0), key, val, 1, NULL, NULL, GET_ALLOCATED_VERSION_PTR(tid, 0));
        assert(recordmgr->isProtected(tid, l));
        /*Node<K,V> *newCopyOfL =*/
        initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), l->key, l->key == NO_KEY ? 0 : 1, NULL, NULL, nullptr);
        initializeNodeWithVersion(tid, GET_ALLOCATED_NODE_PTR(tid, 1), l->key, l->value, l->key == NO_KEY ? 0 : 1, NULL, NULL, GET_ALLOCATED_VERSION_PTR(tid, 1));
        /*Node<K,V> *newInternal;*/
        assert(recordmgr->isProtected(tid, l));
        if (l->key == NO_KEY || cmp(key, l->key)) {
            assert(recordmgr->isProtected(tid, l));
            /*newInternal = */
            initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), l->key, l->key == NO_KEY ? 1 : 2, GET_ALLOCATED_VERSION_PTR(tid, 0), GET_ALLOCATED_VERSION_PTR(tid, 1), nullptr);
            ...
            /*newInternal = */
            initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), key, l->key == NO_KEY ? 1 : 2, GET_ALLOCATED_VERSION_PTR(tid, 1), GET_ALLOCATED_VERSION_PTR(tid, 0), nullptr);
...
bool Chromatic<K,V,Compare,MasterRecordMgr>::updateErase(
    ...
    if (key != l->key) {
        *result = NO_VALUE;
        // TRACE COUTATOMICTSTID("erase("<<key<<") key not found"<<endl);
        return true; // success
    } else {
        assert(key != NO_KEY);
        ...
        *result = l->value;
        ...
        assert(recordmgr->isProtected(tid, s));
        // Version<K>* sleftver = s->left.load(memory_order_relaxed) ? ((Version<K>*) getVersion((Node<K,V>*) s->left.load(memory_order_relaxed))) : NULL;
        // Version<K>* srightver = s->right.load(memory_order_relaxed) ? ((Version<K>*) getVersion((Node<K,V>*) s->right.load(memory_order_relaxed))) : NULL;
        // initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), s->key, ((Version<K>*) getVersion(s))->sum, sleftver, srightver, NULL);
        ...
        *shouldRebalance = (count > N);
        if (SCXAndEnterQuiescentState(tid, SCXRecord<K,V>::TYPE_DEL, nodes, llxResults, (p == gpleft ? &gp->left : &gp->right), GET_ALLOCATED_NODE_PTR(tid, 0))) {
            // TRACE COUTATOMICTSTID("erase("<<key<<") succeeded"<<endl);
            return true; // success
        } else {
            // TRACE COUTATOMICTID("return false13\n");
            return false; // retry
...
template<class K, class V, class Compare, class MasterRecordMgr>
uintptr_t Chromatic<K,V,Compare,MasterRecordMgr>::getVersion(Node<K,V>* x) {
    if ((Version<K>*) x->version.load(memory_order_relaxed) == NULL) {
        nilCASsAttempted[tid * PREFETCH_SIZE_WORDS]++;
        refresh(x, nullptr);
    }
    return x->version.load(memory_order_relaxed);
}

template<class K, class V, class Compare, class MasterRecordMgr>
PropagateStatus* Chromatic<K,V,Compare,MasterRecordMgr>::refresh(Node<K, V>* x, PropagateStatus* ps) {
    uintptr_t oldVer = x->version.load(memory_order_relaxed); // TODO: refresh while value isn't changed?
    if(oldVer != (uintptr_t) nullptr) return nullptr;

    // No need to refresh leaves
    if (!x->left.load(memory_order_relaxed)) {
        Version<K>* leafVer = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), x->key, 1, NULL, NULL, ps);
        x->version.store((uintptr_t) leafVer);
        return ps;
    }
    ...
    // leftver = (Version<K>*) getVersion(left1);
    ...
    do {
        right1 = (Node<K,V>*) x->right.load(memory_order_relaxed);
        rightver = right1 ? (Version<K>*) getVersion(right1) : nullptr;
    } while (right1 != (Node<K,V>*) x->right.load(memory_order_relaxed));

    if (right1) {
        newVer = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), x->key, leftver->sum + rightver->sum, leftver, rightver, ps);
    } else { // root
        newVer = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), x->key, leftver->sum, leftver, NULL, ps);
    }
    ...
    return (PropagateStatus*) ((Version<K>*) oldVer)->status.load(memory_order_relaxed);
}

template<class K, class V, class Compare, class MasterRecordMgr>
PropagateStatus* Chromatic<K,V,Compare,MasterRecordMgr>::refreshUntilConsistentChildren(Node<K, V>* x, PropagateStatus* ps, vector<Version<K>*>& toRetire) {
    uintptr_t oldVer = getVersion(x);
    ...
    Node<K, V> *left1, *left2;
    Version<K>* leftver1, *leftver2;
    Node<K, V> *right1, *right2;
    Version<K>* rightver1, *rightver2;
    Version<K>* newVer;

    do {
        left2 = (Node<K,V>*) x->left.load(memory_order_relaxed);
        leftver2 = (Version<K>*) getVersion(left2);
    } while (left2 != (Node<K,V>*) x->left.load(memory_order_relaxed));
    do {
        right2 = (Node<K,V>*) x->right.load(memory_order_relaxed);
        rightver2 = right2 ? (Version<K>*) getVersion(right2) : nullptr;
    } while (right2 != (Node<K,V>*) x->right.load(memory_order_relaxed));

    // left2 = (Node<K,V>*) x->left.load(memory_order_relaxed);
    // leftver2 = (Version<K>*) getVersion(left2);
    // right2 = (Node<K,V>*) x->right.load(memory_order_relaxed);
    // rightver2 = right2 ? (Version<K>*) getVersion(right2) : NULL;

    do {
        leftver1 = leftver2;
        rightver1 = rightver2;
        if (rightver1) {
            newVer = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), x->key, leftver1->sum + rightver1->sum, leftver1, rightver1, ps);
        } else {
            newVer = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), x->key, leftver1->sum, leftver1, NULL, ps);
        }
    ...
    } while (leftver1 != leftver2 || rightver1 != rightver2);

    // Successful CAS where child versions stayed consistent
    return ps;
}
...
void Chromatic<K,V,Compare,MasterRecordMgr>::propagate(int tid, K key, stack<Node<K,V>*> stack) {
    ...
    ps = initializeStatus(tid, GET_ALLOCATED_STATUS_PTR(tid), nullptr);
    SOFTWARE_BARRIER;
    REPLACE_ALLOCATED_STATUS(tid);
    ...
    while (!set.count(root)) {
        ...
        while (c && ((Node<K,V>*)c->left.load(memory_order_relaxed)) && !set.count(c)) {
        ...
        PropagateStatus* curps = refreshUntilConsistentChildren(r, ps, toRetire);
        if (!r->marked.load(memory_order_relaxed) && curps != ps) {
            ps->delegatee = curps;
            break;
        }
        set.insert(r);
    ...
    PropagateStatus* currDel = ps->delegatee;
    if (currDel != nullptr) {
        while (currDel->getDone() == false) {
            PropagateStatus* next = currDel->getPtr();
            if (next != nullptr) {
                currDel = next;
            }
        }
    ...
    if (highestIndexReached == 0) {
        ...
        if (state == SCXRecord<K,V>::STATE_COMMITTED) {
            ...
            for (int j=1;j<nNodes;++j) {
                ...
                // recordmgr->retire(tid, (Version<K>*) getVersion(nodes[j]));
