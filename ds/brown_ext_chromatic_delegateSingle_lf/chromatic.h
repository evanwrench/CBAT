...
private:
    ...
    // ADDITION
    Version<K> **allocatedVersions;
    #define REPLACE_ALLOCATED_VERSION(tid, i) GET_ALLOCATED_VERSION_PTR(tid, i) = allocateVersion(tid)
    #define GET_ALLOCATED_VERSION_PTR(tid, i) allocatedVersions[tid*(PREFETCH_SIZE_WORDS+MAX_NODES-1)+i]

    PropagateStatus **allocatedStatuses;
    #define REPLACE_ALLOCATED_STATUS(tid) GET_ALLOCATED_STATUS_PTR(tid) = allocateStatus(tid)
    #define GET_ALLOCATED_STATUS_PTR(tid) allocatedStatuses[tid*PREFETCH_SIZE_WORDS]

    vector<tuple<int, uint64_t, int, bool, K, K, int>> *traces;
    ...
    bool updateInsert(const int, const K& key, const V& val, const bool onlyIfAbsent, V *result, bool *shouldRebalance, stack<Node<K,V>*> *stack); // last 2 args are output args
    bool updateErase(const int, const K& key, V *result, bool *shouldRebalance, stack<Node<K,V>*> *stack); // last 2 args are output args
    ...
    uintptr_t getVersion(Node<K,V>*);
    PropagateStatus* refresh(Node<K, V>*, PropagateStatus*);
    PropagateStatus* refreshUntilConsistentChildren(Node<K, V>*, PropagateStatus*, vector<Version<K>*>&);
    void propagate(int tid, K key, stack<Node<K,V>*> stack);
    bool findNode(Version<K>* root, Version<K>* node);
    ...
    unsigned long long basePropagateSizes[MAX_THREADS_POW2 * PREFETCH_SIZE_WORDS] = {};
    unsigned long long totalPropagateSizes[MAX_THREADS_POW2 * PREFETCH_SIZE_WORDS] = {};
    unsigned long long nilCASsAttempted[MAX_THREADS_POW2 * PREFETCH_SIZE_WORDS] = {};
    unsigned long long totCASsAttempted[MAX_THREADS_POW2 * PREFETCH_SIZE_WORDS] = {};
    vector<Version<K>*>** versToRetire;
...
public:
    ...
    void dfsDeallocateBottomUp(Node<K,V> * const u, set<void*>& seen, int *numNodes) {
        ...
        recordmgr->deallocate(0, (Version<K>*) u->version.load(memory_order_relaxed)); // Addition
        recordmgr->deallocate(0 /* tid */, u);
    }
    string getLogString(tuple<int, uint64_t, int, bool, K, K, int> tup) {
        string type = get<2>(tup)==0 ? "Insert" : get<2>(tup)==1 ? "Delete" : get<2>(tup)==2 ? "Find" : "RangeQuery";
        if (get<5>(tup) == -1) { // Insert/Delete/Find
            if (get<3>(tup)) { // Response
                return "tid="+to_string(get<0>(tup))+" cycle="+to_string(get<1>(tup))+" "+type+" "+to_string(get<4>(tup))+" returned "+to_string(get<6>(tup))+"\n";
            } else { // Invocation
                return "tid="+to_string(get<0>(tup))+" cycle="+to_string(get<1>(tup))+" "+type+" "+to_string(get<4>(tup))+" started\n";
            }
        } else { // RangeQuery
            if (get<3>(tup)) { // Response
                return "tid="+to_string(get<0>(tup))+" cycle="+to_string(get<1>(tup))+" RangeQuery ["+to_string(get<4>(tup))+","+to_string(get<5>(tup))+"] returned "+to_string(get<6>(tup))+"\n";
            } else { // Invocation
                return "tid="+to_string(get<0>(tup))+" cycle="+to_string(get<1>(tup))+" RangeQuery ["+to_string(get<4>(tup))+","+to_string(get<5>(tup))+"] started\n";
            }
        }
        
    }
    static bool comp(const tuple<int, uint64_t, int, bool, K, K, int> &a, const tuple<int, uint64_t, int, bool, K, K, int> &b) {
        return get<1>(a) < get<1>(b);
    }
    ~Chromatic() {
        VERBOSE DEBUG COUTATOMIC("destructor chromatic");

        vector<tuple<int, uint64_t, int, bool, K, K, int>> combined;
        int size = 0;
        unsigned long long numProp = 0;
        unsigned long long totRef = 0;
        unsigned long long baseProp = 0;
        unsigned long long totProp = 0;
        unsigned long long nilCAS = 0;
        unsigned long long totCAS = 0;
        for(int i = 0; i < recordmgr->NUM_PROCESSES; i++) {
            size+=traces[i].size();
            numProp+=numPropagates[i * PREFETCH_SIZE_WORDS];
            totRef+=numLevelsPropagated[i * PREFETCH_SIZE_WORDS];
            baseProp+=basePropagateSizes[i * PREFETCH_SIZE_WORDS];
            totProp+=totalPropagateSizes[i * PREFETCH_SIZE_WORDS];
            nilCAS+=nilCASsAttempted[i * PREFETCH_SIZE_WORDS];
            totCAS+=totCASsAttempted[i * PREFETCH_SIZE_WORDS];
        }
        COUTATOMIC("Avg_nodes_per_propagate="<<(((long double)totRef) / numProp)<<endl);
        COUTATOMIC("Avg_total_over_base_nodes_propagated="<<(((long double)totProp) / baseProp)<<endl);
        COUTATOMIC("Avg_nil_cas_per_propagate="<<(((long double)nilCAS) / numProp)<<endl);
        COUTATOMIC("Avg_total_cas_per_propagate="<<(((long double)totCAS) / numProp)<<endl);
        if (size > 0) {
            combined.reserve(size);
            for(int i = 0; i < recordmgr->NUM_PROCESSES; i++) {
                combined.insert(combined.end(), traces[i].begin(), traces[i].end());
            }
            sort(combined.begin(), combined.end(), comp);
            ofstream tracefile;
            tracefile.open("traces/delegateSingle.txt", ios::app);
            for (int i = 0; i < size; i++) {
                tracefile<<getLogString(combined[i]);
            }
            tracefile.close();
        }
        ...
        for (int tid=0;tid<recordmgr->NUM_PROCESSES;++tid) {
            for (int i=0;i<MAX_NODES-1;++i) {
                recordmgr->deallocate(tid, GET_ALLOCATED_VERSION_PTR(tid, i)); // Addition
                recordmgr->deallocate(tid, GET_ALLOCATED_NODE_PTR(tid, i));
            }
            recordmgr->deallocate(tid, GET_ALLOCATED_STATUS_PTR(tid));
            recordmgr->deallocate(tid, GET_ALLOCATED_SCXRECORD_PTR(tid));
            delete versToRetire[tid * PREFETCH_SIZE_WORDS];
        }
        delete recordmgr;
    ...
    const int getRank(const int tid, const K& key);
    const int doGetRank(const int tid, const K& key, Version<K>* curr);
    const K selectItem(const int tid, int rank);
    const K doSelectItem(const int tid, int rank, Version<K>* curr);
    const int rangeQuery(const int tid, const K& lo, const K& hi);
    const int doRangeQuery(const int tid, const K& lo, const K& hi, Version<K>* curr);
    const bool compareKeys(const K& a, bool aIsLo, const K& b, bool bIsLo);
    ...
    void printExpectedSum() {
        Version<K>* rootleftver = (Version<K>*) ((Node<K,V>*) root->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
        COUTATOMIC("EXPECTED SUM BY VERSION: "<<rootleftver->sum<<endl);
    }
    // void clearCounters() {
    //     counters.clear();
