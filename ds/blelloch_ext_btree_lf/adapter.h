...
#endif
#include "new_ordered_map.h"


// #define RECORD_MANAGER_T record_manager<reclaimer_debra_bundle<K>, Alloc, Pool, node>
...
public:
    ...
    bool contains(const int tid, const K& key) {
        return ds->find(key).has_value();
    }
    V insert(const int tid, const K& key, const V& val) {
        return ds->insert(key, val) ? NO_VALUE : val;
    }
    ...
    V find(const int tid, const K& key) {
        return ds->find(key).value_or(NULL);
    }
