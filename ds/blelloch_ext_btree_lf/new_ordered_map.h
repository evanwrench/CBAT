#include "../../include/verlib/verlib.h"
...
struct ordered_map {
  ...
  static bool range_internal(node* a, AddF& add, const K& start, const K& end, bool &found) {
    while (true) {
      if (!a) return false;
      if (a->is_leaf) {
    leaf* la = (leaf*) a;
    ...
    int e = la->prev(end, s);
    for (int i = s; i < e; i++) {
      if (la->keyvals[i].key == end) found = true;
    ...
    for (int i = s; i <= e; i++) {
      if (!range_internal(a->children[i].read_snapshot(), add, start, end, found))
        return false;
    }
    return true;
      }
  ...
  template<typename AddF>
  void rank_(AddF& add, const K& key, bool &found) {
    range_internal(root, add, 0, key, found);
  }

  template<typename AddF>
  void range_(AddF& add, const K& start, const K& end, bool &found) {
      range_internal(root, add, start, end, found);
  }
