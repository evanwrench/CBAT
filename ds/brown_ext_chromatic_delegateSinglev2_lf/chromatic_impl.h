...
#define initializeVersion(_tid, _newver, _key, _sum, _left, _right, _status, _isSecond) \
(_newver); \
{ \
    (_newver)->key = (_key); \
    (_newver)->sum = (_sum); \
    (_newver)->isSecondCas = (_isSecond); \
...
{ \
    ...
    if((_left) == nullptr) { \
        ...
        initializeVersion(_tid, _vernode, _key, 1, nullptr, nullptr, nullptr, false); \
        (_newnode)->version.store((uintptr_t) (_vernode), memory_order_relaxed); \
    } \
...
Chromatic<K,V,Compare,MasterRecordMgr>::Chromatic(const K& _NO_KEY,
    ...
    initializeVersion(tid, rootleftver, NO_KEY, 0, NULL, NULL, nullptr, false);
    Version<K>* rootver = allocateVersion(tid);
    initializeVersion(tid, rootver, NO_KEY, 0, rootleftver, NULL, nullptr, false);
...
bool Chromatic<K,V,Compare,MasterRecordMgr>::contains(const int tid, const K& key) {
    ...
    Version<K>* l = (Version<K>*) ((Node<K,V>*) root->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
...
//            assert(!recordmgr->isProtected(tid, p));
    ...
    if (key == l->key) {
        ...
        *result = l->value;
        ...
        initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), key, 1 /* sum */, NULL, NULL, nullptr, false);
        ...
        /*Node<K,V> *newLeaf =*/
        initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), key, 1, NULL, NULL, nullptr, false);
        initializeNodeWithVersion(tid, GET_ALLOCATED_NODE_PTR(tid, 0), key, val, 1, NULL, NULL, GET_ALLOCATED_VERSION_PTR(tid, 0));
        assert(recordmgr->isProtected(tid, l));
        /*Node<K,V> *newCopyOfL =*/
        initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), l->key, l->key == NO_KEY ? 0 : 1, NULL, NULL, nullptr, false);
        ...
        if (l->key == NO_KEY || cmp(key, l->key)) {
            assert(recordmgr->isProtected(tid, l));
            /*newInternal = */
            initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), l->key, l->key == NO_KEY ? 1 : 2, GET_ALLOCATED_VERSION_PTR(tid, 0), GET_ALLOCATED_VERSION_PTR(tid, 1), nullptr, false);
            ...
            /*newInternal = */
            initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), key, l->key == NO_KEY ? 1 : 2, GET_ALLOCATED_VERSION_PTR(tid, 1), GET_ALLOCATED_VERSION_PTR(tid, 0), nullptr, false);
...
bool Chromatic<K,V,Compare,MasterRecordMgr>::updateErase(
    ...
    if (key != l->key) {
        ...
        *result = l->value;
        ...
        // initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), s->key, ((Version<K>*) getVersion(s))->sum, sleftver, srightver, NULL, false);
...
PropagateStatus* Chromatic<K,V,Compare,MasterRecordMgr>::refresh(Node<K, V>* x, PropagateStatus* ps) {
    ...
    Node<K, V> *right1, *right2;
    Version<K> *rightver;
    Version<K> *newVer = nullptr;
    left1 = (Node<K,V>*) x->left.load(memory_order_relaxed);
    leftver = (Version<K>*) getVersion(left1);
    right1 = (Node<K,V>*) x->right.load(memory_order_relaxed);
    if (right1) {
        rightver = (Version<K>*) getVersion(right1);
        newVer = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), x->key, leftver->sum + rightver->sum, leftver, rightver, ps, false);
    } else { // root
        newVer = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), x->key, leftver->sum, leftver, NULL, ps, false);
    }
...
template<class K, class V, class Compare, class MasterRecordMgr>
PropagateStatus* Chromatic<K,V,Compare,MasterRecordMgr>::refreshLong(Node<K, V>* x, PropagateStatus* ps, vector<Version<K>*>& toRetire) {
    ...
    Version<K> *newVer1, *newVer2;

    left1 = (Node<K,V>*) x->left.load(memory_order_relaxed);
    leftver1 = (Version<K>*) getVersion(left1);
    right1 = (Node<K,V>*) x->right.load(memory_order_relaxed);
    if (right1) {
        rightver1 = (Version<K>*) getVersion(right1);
        newVer1 = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), x->key, leftver1->sum + rightver1->sum, leftver1, rightver1, ps, false);
    } else { // root
        rightver1 = NULL;
        newVer1 = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), x->key, leftver1->sum, leftver1, NULL, ps, false);
    }
    
    SOFTWARE_BARRIER;
    REPLACE_ALLOCATED_VERSION(tid, 0);
    SOFTWARE_BARRIER;
    bool res = x->version.compare_exchange_strong(oldVer, (uintptr_t) newVer1);
    if (res) {
        ...
        if (leftver1 == leftver2 && rightver1 == rightver2) {
            return ps;
        } else {
            if (rightver2) {
                newVer2 = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), x->key, leftver2->sum + rightver2->sum, leftver2, rightver2, ps, true);
            } else {
                newVer2 = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), x->key, leftver2->sum, leftver2, NULL, ps, true);
            }
            SOFTWARE_BARRIER;
            REPLACE_ALLOCATED_VERSION(tid, 0);
            SOFTWARE_BARRIER;
            oldVer = (uintptr_t) newVer1;
            bool res2 = x->version.compare_exchange_strong(oldVer, (uintptr_t) newVer2);
            if (res2) {
                if(oldVer) toRetire.push_back((Version<K>*) oldVer);
                return ps;
            } else {
                toRetire.push_back(newVer2);
                return (PropagateStatus*) ((Version<K>*) oldVer)->status.load(memory_order_relaxed);
            }
        }
    } else {
        if (!(((Version<K>*) oldVer)->isSecondCas)) { // Interrupted by a "first" CAS
            toRetire.push_back(newVer1);
            return (PropagateStatus*) ((Version<K>*) oldVer)->status.load(memory_order_relaxed);
        } else {
            left2 = (Node<K,V>*) x->left.load(memory_order_relaxed);
            leftver2 = (Version<K>*) getVersion(left2);
            ...
            if (res2) {
                ...
                toRetire.push_back(newVer2);
                return (PropagateStatus*) ((Version<K>*) newVer1)->status.load(memory_order_relaxed);
            }
        }
    }
}
...
template<class K, class V, class Compare, class MasterRecordMgr>
void Chromatic<K,V,Compare,MasterRecordMgr>::propagate(K key) {

    vector<Version<K>*> toRetire;
    toRetire.reserve(50);
    PropagateStatus *ps;
    ...
    while (!set.count(root)) {
        ...
        PropagateStatus* curps = refreshLong(r, ps, toRetire);
        if (r->marked.load(memory_order_relaxed)) {
            refreshLong(r, ps, toRetire);
        } else {
            if (curps != ps) {
                ps->delegatee = curps;
                break;
