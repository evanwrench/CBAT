...
#define allocateVersion(tid) recordmgr->template allocate<Version<K>> ((tid))
...
// TODO: keep (_newver)?
#define initializeVersion(_tid, _newver, _key, _sum, _left, _right) \
(_newver); \
{ \
    (_newver)->key = (_key); \
    (_newver)->sum = (_sum); \
    (_newver)->left.store((uintptr_t) (_left), memory_order_relaxed); \
    (_newver)->right.store((uintptr_t) (_right), memory_order_relaxed); \
}

#define initializeNodeWithVersion(_tid, _newnode, _key, _value, _weight, _left, _right, _vernode) \
(_newnode); \
{ \
    (_newnode)->key = (_key); \
    ...
    (_newnode)->version.store((uintptr_t) (_vernode), memory_order_relaxed); \
}
...
{ \
    ...
    (_newnode)->marked.store(false, memory_order_relaxed); \
    if((_left) == nullptr) { \
        Version<K>* _vernode = allocateVersion(tid); \
        initializeVersion(_tid, _vernode, _key, 1, nullptr, nullptr); \
        (_newnode)->version.store((uintptr_t) (_vernode), memory_order_relaxed); \
    } \
    else (_newnode)->version.store((uintptr_t) nullptr, memory_order_relaxed); \
}
...
Chromatic<K,V,Compare,MasterRecordMgr>::Chromatic(const K& _NO_KEY,
    ...
    allocatedVersions = new Version<K>*[numProcesses*(PREFETCH_SIZE_WORDS+MAX_NODES-1)];
    versToRetire = new vector<Version<K>*>*[numProcesses * PREFETCH_SIZE_WORDS];
    for (int tid=0;tid<numProcesses;++tid) {
        versToRetire[tid * PREFETCH_SIZE_WORDS] = new vector<Version<K>*>;
        versToRetire[tid * PREFETCH_SIZE_WORDS]->reserve(50);
    ...
    // Addition
    Version<K>* rootleftver = allocateVersion(tid);
    initializeVersion(tid, rootleftver, NO_KEY, 0, NULL, NULL);
    Version<K>* rootver = allocateVersion(tid);
    initializeVersion(tid, rootver, NO_KEY, 0, rootleftver, NULL);
    
    Node<K,V> *rootleft = allocateNode(tid);
    initializeNodeWithVersion(tid, rootleft, NO_KEY, NO_VALUE, 1, NULL, NULL, rootleftver);
    root = allocateNode(tid);
    initializeNodeWithVersion(tid, root, NO_KEY, NO_VALUE, 1, rootleft, NULL, rootver);
}
...
void Chromatic<K,V,Compare,MasterRecordMgr>::initThread(const int tid) {
    ...
    if (GET_ALLOCATED_SCXRECORD_PTR(tid) == NULL) {
        ...
        for (int i=0;i<MAX_NODES-1;++i) {
            REPLACE_ALLOCATED_VERSION(tid, i); // Addition
...
bool Chromatic<K,V,Compare,MasterRecordMgr>::contains(const int tid, const K& key) {
    // TRACE traces[tid].push_back(tuple<int, uint64_t, int, bool, K, K, int>(tid, GETTIMESTAMP(), 2, 0, key, -1, -1));
    auto guard = recordmgr->getGuard(tid, true);
    Version<K>* l = (Version<K>*) root->version.load(memory_order_relaxed);
    while ((Version<K>*) l->left.load(memory_order_relaxed) != NULL) {
        if (l->key == NO_KEY || cmp(key, l->key)) {
            l = (Version<K>*) l->left.load(memory_order_relaxed);
        } else {
            l = (Version<K>*) l->right.load(memory_order_relaxed);
        }
    }
    if (key == l->key) {
        // TRACE traces[tid].push_back(tuple<int, uint64_t, int, bool, K, K, int>(tid, GETTIMESTAMP(), 2, 1, key, -1, 1));
        return true;
    }
    // TRACE traces[tid].push_back(tuple<int, uint64_t, int, bool, K, K, int>(tid, GETTIMESTAMP(), 2, 1, key, -1, 0));
    return false;
}

template<class K, class V, class Compare, class MasterRecordMgr>
const pair<V,bool> Chromatic<K,V,Compare,MasterRecordMgr>::find(const int tid, const K& key) {
    TRACE COUTATOMICTSTID("find(tid="<<tid<<" key="<<key<<")"<<endl);
    pair<V,bool> result;
...
template<class K, class V, class Compare, class MasterRecordMgr>
const int Chromatic<K,V,Compare,MasterRecordMgr>::rangeQuery(const int tid, const K& lo, const K& hi) {
    TRACE COUTATOMICTSTID("rangeQuery(tid="<<tid<<" lo="<<lo<<" hi="<<hi<<")"<<endl);
    auto guard = recordmgr->getGuard(tid, true);
    int res = doRangeQuery(tid, lo, hi, (Version<K>*) root->version.load(memory_order_relaxed));
    TRACE COUTATOMICTSTID("rangeQuery returned "<<res<<endl);
    return res;
}

template<class K, class V, class Compare, class MasterRecordMgr>
const int Chromatic<K,V,Compare,MasterRecordMgr>::getRank(const int tid, const K& key) {
    auto guard = recordmgr->getGuard(tid, true);
    return doGetRank(tid, key, (Version<K>*) root->version.load(memory_order_relaxed));
}

template<class K, class V, class Compare, class MasterRecordMgr>
const int Chromatic<K,V,Compare,MasterRecordMgr>::doGetRank(const int tid, const K& key, Version<K>* curr) {
    if (((Version<K>*) curr->left.load(memory_order_relaxed)) == NULL) {
        if (curr->key == key) {
            return 1;
        } else {
    ...
    Version<K>* currLeft = (Version<K>*)curr->left.load(memory_order_relaxed);
    Version<K>* currRight = (Version<K>*)curr->left.load(memory_order_relaxed);
    if (curr->key == NO_KEY || cmp(key, curr->key)) {
        return doGetRank(tid, key, currLeft);
    }
    return currLeft->sum + doGetRank(tid, key, currRight);
}

template<class K, class V, class Compare, class MasterRecordMgr>
const int Chromatic<K,V,Compare,MasterRecordMgr>::doRangeQuery(const int tid, const K& lo, const K& hi, const Version<K>* curr) {

    // if (curr == NULL || ((Version<K>*)curr->left.load(memory_order_relaxed)) == NULL) {
    //     return 1;
    // }
    // return doRangeQuery(tid, lo, hi, (Version<K>*) curr->left.load(memory_order_relaxed)) + doRangeQuery(tid, lo, hi, (Version<K>*) curr->right.load(memory_order_relaxed));

    if (lo == NO_KEY && hi == NO_KEY) {
        return curr->sum;
    }
    if (((Version<K>*) curr->left.load(memory_order_relaxed)) == NULL) { // Leaf
        return (curr->key != NO_KEY && !compareKeys(curr->key, false, lo, true) && !compareKeys(hi, false, curr->key, false)) ? 1 : 0;
    }
    if (curr->key == NO_KEY || compareKeys(hi, false, curr->key, false)) {
        return doRangeQuery(tid, lo, hi, (Version<K>*) curr->left.load(memory_order_relaxed));
    }
    if (compareKeys(curr->key, false, lo, true)) {
        return doRangeQuery(tid, lo, hi, (Version<K>*) curr->right.load(memory_order_relaxed));
    }
    return doRangeQuery(tid, lo, NO_KEY, (Version<K>*) curr->left.load(memory_order_relaxed)) + doRangeQuery(tid, NO_KEY, hi, (Version<K>*) curr->right.load(memory_order_relaxed));
}

// Defenitely a better way to do this, but this function compares 2 keys to see if a < b
// If either one is lo for rangeQuery, we take NULL to be -inf, otherwise we take 
// NULL to be +inf (can happen if one of the keys is hi from rangeQuery)
template<class K, class V, class Compare, class MasterRecordMgr>
const bool Chromatic<K,V,Compare,MasterRecordMgr>::compareKeys(const K& a, bool aIsLo, const K& b, bool bIsLo) {
    if (a == NO_KEY) {
        return aIsLo ? 1 : 0;
    } else if (b == NO_KEY) {
        return bIsLo ? 0 : 1;
...
const V Chromatic<K,V,Compare,MasterRecordMgr>::insert(const int tid, const K& key, const V& val) {
    ...
    while (!finished) {
        ...
        CHECKPOINT_AND_RUN_UPDATE(tid, finished) {
            finished = updateInsert(tid, key, val, onlyIfAbsent, &result, &shouldRebalance, &stack);
    ...
    DEBUG COUTATOMIC("SUCCESSFUL INSERT OF KEY: "<<key<<endl);
    // DEBUG COUTATOMIC("SUM IS: "<<((Version<K>*) getVersion((Node<K,V>*)root->left.load(memory_order_relaxed)))->sum<<endl);
    IFREBALANCING if (shouldRebalance) {
        DEBUG COUTATOMIC("rebalancing");
        fixAllToKey(tid, key);
        // DEBUG COUTATOMIC("AFTER REBALANCE: "<<((Version<K>*) getVersion((Node<K,V>*)root->left.load(memory_order_relaxed)))->sum<<endl);
    }
...
const V Chromatic<K,V,Compare,MasterRecordMgr>::insertIfAbsent(const int tid, const K& key, const V& val) {
    // TRACE COUTATOMICTSTID("insert(tid="<<tid<<", key="<<key<<")"<<endl);
    // TRACE traces[tid].push_back(tuple<int, uint64_t, int, bool, K, K, int>(tid, GETTIMESTAMP(), 0, 0, key, -1, -1));
    ...
    // this is considered to be a whole new operation (possibly many, in fact).
    DEBUG COUTATOMIC("SUCCESSFUL INSERT OF KEY: "<<key<<endl);
    DEBUG COUTATOMIC("SUM IS: "<<((Version<K>*)((Node<K,V>*)root->left.load(memory_order_relaxed))->version.load(memory_order_relaxed))->sum<<endl);
    IFREBALANCING if (shouldRebalance) {
        DEBUG COUTATOMIC("rebalancing");
        fixAllToKey(tid, key);
        DEBUG COUTATOMIC("AFTER REBALANCE: "<<((Version<K>*)((Node<K,V>*)root->left.load(memory_order_relaxed))->version.load(memory_order_relaxed))->sum<<endl);
    }
    propagate(tid, key, stack);
    if (result == NO_VALUE) { // key not found in tree
        // TRACE COUTATOMICTSTID("insert("<<key<<") succeeded"<<endl);
        // TRACE traces[tid].push_back(tuple<int, uint64_t, int, bool, K, K, int>(tid, GETTIMESTAMP(), 0, 1, key, -1, 1));
    } else { // key found in tree
        // TRACE COUTATOMICTSTID("insert("<<key<<") found pre-existing key"<<endl);
        // TRACE traces[tid].push_back(tuple<int, uint64_t, int, bool, K, K, int>(tid, GETTIMESTAMP(), 0, 1, key, -1, 0));
    }
    return result;
}
...
const pair<V,bool> Chromatic<K,V,Compare,MasterRecordMgr>::erase(const int tid, const K& key) {
    // TRACE COUTATOMICTSTID("delete(tid="<<tid<<", key="<<key<<")"<<endl);
    // TRACE traces[tid].push_back(tuple<int, uint64_t, int, bool, K, K, int>(tid, GETTIMESTAMP(), 1, 0, key, -1, -1));
    V result = NO_VALUE;
    bool shouldRebalance = false;
    bool finished = false;
    stack<Node<K,V>*> stack;
    ...
    while (!finished) {
        while(!stack.empty()) stack.pop();
        CHECKPOINT_AND_RUN_UPDATE(tid, finished) {
            finished = updateErase(tid, key, &result, &shouldRebalance, &stack);
            // if (!finished) ++counters[tid]->eraseRestart;
        }
    }
    // call another routine to handle rebalancing.
    // this is considered to be a whole new operation (possibly many, in fact).
    DEBUG COUTATOMIC("SUCCESSFUL DELETE OF KEY"<<key<<endl);
    DEBUG COUTATOMIC("SUM IS: "<<((Version<K>*)((Node<K,V>*)root->left.load(memory_order_relaxed))->version.load(memory_order_relaxed))->sum<<endl);
    IFREBALANCING if (shouldRebalance) {
        DEBUG COUTATOMIC("rebalancing");
        fixAllToKey(tid, key);
    ...
    propagate(tid, key, stack);
    if (result == NO_VALUE) { // key not found in tree
        // TRACE COUTATOMICTSTID("delete("<<key<<") did not find key in tree"<<endl);
        // TRACE traces[tid].push_back(tuple<int, uint64_t, int, bool, K, K, int>(tid, GETTIMESTAMP(), 1, 1, key, -1, 0));
    } else { // key found in tree
        // TRACE COUTATOMICTSTID("delete("<<key<<") succeeded"<<endl);
        // TRACE traces[tid].push_back(tuple<int, uint64_t, int, bool, K, K, int>(tid, GETTIMESTAMP(), 1, 1, key, -1, 1));
    }
...
//            assert(!recordmgr->isProtected(tid, p));
    ...
    if (key == l->key) {
        ...
        *result = l->value;
        ...
        initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), key, 1 /* sum */, NULL, NULL);
        initializeNodeWithVersion(tid, GET_ALLOCATED_NODE_PTR(tid, 0), key, val, l->weight, NULL, NULL, GET_ALLOCATED_VERSION_PTR(tid, 0));
        REPLACE_ALLOCATED_VERSION(tid, 0);
        ...
        /*Node<K,V> *newLeaf =*/
        initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), key, 1, NULL, NULL);
        initializeNodeWithVersion(tid, GET_ALLOCATED_NODE_PTR(tid, 0), key, val, 1, NULL, NULL, GET_ALLOCATED_VERSION_PTR(tid, 0));
        assert(recordmgr->isProtected(tid, l));
        /*Node<K,V> *newCopyOfL =*/
        initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1),l->key, l->key == NO_KEY ? 0 : 1, NULL, NULL);
        initializeNodeWithVersion(tid, GET_ALLOCATED_NODE_PTR(tid, 1), l->key, l->value, 1, NULL, NULL, GET_ALLOCATED_VERSION_PTR(tid, 1));
        /*Node<K,V> *newInternal;*/
        assert(recordmgr->isProtected(tid, l));
        if (l->key == NO_KEY || cmp(key, l->key)) {
            assert(recordmgr->isProtected(tid, l));
            /*newInternal = */
            initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), l->key, l->key == NO_KEY ? 1 : 2, GET_ALLOCATED_VERSION_PTR(tid, 0), GET_ALLOCATED_VERSION_PTR(tid, 1));
            initializeNodeWithVersion(tid, GET_ALLOCATED_NODE_PTR(tid, 2), l->key, l->value, newWeight, GET_ALLOCATED_NODE_PTR(tid, 0), GET_ALLOCATED_NODE_PTR(tid, 1), GET_ALLOCATED_VERSION_PTR(tid, 2));
        } else {
            /*newInternal = */
            initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), key, l->key == NO_KEY ? 1 : 2, GET_ALLOCATED_VERSION_PTR(tid, 1), GET_ALLOCATED_VERSION_PTR(tid, 0));
            initializeNodeWithVersion(tid, GET_ALLOCATED_NODE_PTR(tid, 2), key, val, newWeight, GET_ALLOCATED_NODE_PTR(tid, 1), GET_ALLOCATED_NODE_PTR(tid, 0), GET_ALLOCATED_VERSION_PTR(tid, 2));
        }
        REPLACE_ALLOCATED_VERSION(tid, 0);
        REPLACE_ALLOCATED_VERSION(tid, 1);
        REPLACE_ALLOCATED_VERSION(tid, 2);
        *result = NO_VALUE;
...
bool Chromatic<K,V,Compare,MasterRecordMgr>::updateErase(
            ...
            bool * const shouldRebalance,
            stack<Node<K,V>*> *stack) {
    // TRACE COUTATOMICTSTID("erase(tid="<<tid<<", key="<<key<<")"<<endl);
    ...
    if ((Node<K,V>*) l->left.load(memory_order_relaxed) == NULL) {
        // TRACE COUTATOMICTSTID("erase("<<key<<") found empty tree"<<endl);
        return true;
    ...
    if (key != l->key) {
        ...
        *result = l->value;
        ...
        assert(recordmgr->isProtected(tid, s));
        // Version<K>* sleftver = s->left.load(memory_order_relaxed) ? ((Version<K>*) ((Node<K,V>*) s->left.load(memory_order_relaxed))->version.load(memory_order_relaxed)) : NULL;
        // Version<K>* srightver = s->right.load(memory_order_relaxed) ? ((Version<K>*) ((Node<K,V>*) s->right.load(memory_order_relaxed))->version.load(memory_order_relaxed)) : NULL;
        // initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), s->key, ((Version<K>*) s->version.load(memory_order_relaxed))->sum, sleftver, srightver);
...
template<class K, class V, class Compare, class MasterRecordMgr> bool Chromatic<K,V,Compare,MasterRecordMgr>::doBlk(const int tid, Node<K,V> **nodes, void **llxResults, bool fieldIsLeft) {
    ...
    // Version<K>* verXLleft = (Version<K>*) ((Node<K,V>*) nodes[2]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXLright = (Version<K>*) ((Node<K,V>*) nodes[2]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXRleft = (Version<K>*) ((Node<K,V>*) nodes[3]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXRright = (Version<K>*) ((Node<K,V>*) nodes[3]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);

    // No need to recompute sum since we are not rotating nodes
    // Version<K>* nodeXLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), nodes[2]->key, ((Version<K>*) nodes[2]->version.load(memory_order_relaxed))->sum, verXLleft, verXLright);
    // Version<K>* nodeXRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), nodes[3]->key, ((Version<K>*) nodes[3]->version.load(memory_order_relaxed))->sum, verXRleft, verXRright);
    ...
    // Version<K>* nodeXver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), nodes[1]->key, ((Version<K>*) nodes[1]->version.load(memory_order_relaxed))->sum, nodeXLver, nodeXRver);
...
template<class K, class V, class Compare, class MasterRecordMgr> bool Chromatic<K,V,Compare,MasterRecordMgr>::doRb1(const int tid, Node<K,V> **nodes, void **llxResults, bool fieldIsLeft) {
    ...
    // Version<K>* verXRleft = (Version<K>*) ((Node<K,V>*) nodes[2]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXRright = (Version<K>*) ((Node<K,V>*) nodes[1]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXleft = (Version<K>*) ((Node<K,V>*) nodes[2]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);

    // Version<K>* nodeXRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), nodes[1]->key, verXRleft->sum + verXRright->sum, verXRleft, verXRright);
    ...
    // Version<K>* nodeXver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), nodes[2]->key, verXleft->sum + nodeXRver->sum, verXleft, nodeXRver);
...
template<class K, class V, class Compare, class MasterRecordMgr> bool Chromatic<K,V,Compare,MasterRecordMgr>::doRb2(const int tid, Node<K,V> **nodes, void **llxResults, bool fieldIsLeft) {
    ...
    // Version<K>* verXLleft = (Version<K>*) ((Node<K,V>*) nodes[2]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXLright = (Version<K>*) ((Node<K,V>*) nodes[3]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXRleft = (Version<K>*) ((Node<K,V>*) nodes[3]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXRright = (Version<K>*) ((Node<K,V>*) nodes[1]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);

    // Version<K>* nodeXLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), nodes[2]->key, verXLleft->sum + verXLright->sum, verXLleft, verXLright);
    // Version<K>* nodeXRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), nodes[1]->key, verXRleft->sum + verXRright->sum, verXRleft, verXRright);
    ...
    int weight = nodes[1]->weight;
    // Version<K>* nodeXver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), nodes[3]->key, nodeXLver->sum + nodeXRver->sum, nodeXLver, nodeXRver);
...
template<class K, class V, class Compare, class MasterRecordMgr> bool Chromatic<K,V,Compare,MasterRecordMgr>::doPush(const int tid, Node<K,V> **nodes, void **llxResults, bool fieldIsLeft) {
    ...
    // Version<K>* verXXLleft = (Version<K>*) ((Node<K,V>*) nodes[2]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXLright = (Version<K>*) ((Node<K,V>*) nodes[2]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRleft = (Version<K>*) ((Node<K,V>*) nodes[3]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRright = (Version<K>*) ((Node<K,V>*) nodes[3]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    
    // No need to recompute sum since we are not rotating nodes
    // Version<K>* nodeXXLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), nodes[2]->key, ((Version<K>*) nodes[2]->version.load(memory_order_relaxed))->sum, verXXLleft, verXXLright);
    // Version<K>* nodeXXRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), nodes[3]->key, ((Version<K>*) nodes[3]->version.load(memory_order_relaxed))->sum, verXXRleft, verXXRright);
    ...
    int weight = (IS_SENTINEL(nodes[1], nodes[0]) ? 1 : nodes[1]->weight+1); // root of old subtree is a sentinel
    // Version<K>* nodeXXver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), nodes[1]->key, ((Version<K>*) nodes[1]->version.load(memory_order_relaxed))->sum, nodeXXLver, nodeXXRver);
...
template<class K, class V, class Compare, class MasterRecordMgr> bool Chromatic<K,V,Compare,MasterRecordMgr>::doW1(const int tid, Node<K,V> **nodes, void **llxResults, bool fieldIsLeft) {
    ...
    // Version<K>* verXXLLleft = (Version<K>*) ((Node<K,V>*) nodes[2]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXLLright = (Version<K>*) ((Node<K,V>*) nodes[2]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXLRleft = (Version<K>*) ((Node<K,V>*) nodes[4]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXLRright = (Version<K>*) ((Node<K,V>*) nodes[4]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXright = (Version<K>*) ((Node<K,V>*) nodes[3]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    
    // Version<K>* nodeXXLLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), nodes[2]->key, verXXLLleft->sum+verXXLLright->sum, verXXLLleft, verXXLLright);
    // Version<K>* nodeXXLRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 3), nodes[4]->key, verXXLRleft->sum+verXXLRright->sum, verXXLRleft, verXXLRright);
    // Version<K>* nodeXXLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), nodes[1]->key, nodeXXLLver->sum+nodeXXLRver->sum, nodeXXLLver, nodeXXLRver);
    ...
    int weight = nodes[1]->weight;
    // Version<K>* nodeXXver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), nodes[3]->key, nodeXXLver->sum+verXXright->sum, nodeXXLver, verXXright);
...
template<class K, class V, class Compare, class MasterRecordMgr> bool Chromatic<K,V,Compare,MasterRecordMgr>::doW3(const int tid, Node<K,V> **nodes, void **llxResults, bool fieldIsLeft) {
    ...
    // Version<K>* verXXLLLleft = (Version<K>*) ((Node<K,V>*) nodes[2]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXLLLright = (Version<K>*) ((Node<K,V>*) nodes[2]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXLLright = (Version<K>*) ((Node<K,V>*) nodes[5]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXLRleft = (Version<K>*) ((Node<K,V>*) nodes[5]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXLRright = (Version<K>*) ((Node<K,V>*) nodes[4]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXright = (Version<K>*) ((Node<K,V>*) nodes[3]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);

    // Version<K>* nodeXXLLLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 4), nodes[2]->key, verXXLLLleft->sum+verXXLLLright->sum, verXXLLLleft, verXXLLLright);
    // Version<K>* nodeXXLLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), nodes[1]->key, nodeXXLLLver->sum+verXXLLright->sum, nodeXXLLLver, verXXLLright);
    // Version<K>* nodeXXLRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 3), nodes[4]->key, verXXLRleft->sum+verXXLRright->sum, verXXLRleft, verXXLRright);
    // Version<K>* nodeXXLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), nodes[5]->key, nodeXXLLver->sum+nodeXXLRver->sum, nodeXXLLver, nodeXXLRver);
...
template<class K, class V, class Compare, class MasterRecordMgr> bool Chromatic<K,V,Compare,MasterRecordMgr>::doW4(const int tid, Node<K,V> **nodes, void **llxResults, bool fieldIsLeft) {
    ...
    // Version<K>* verXXLLleft = (Version<K>*) ((Node<K,V>*) nodes[2]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXLLright = (Version<K>*) ((Node<K,V>*) nodes[2]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXLright = (Version<K>*) ((Node<K,V>*) nodes[4]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRLleft = (Version<K>*) ((Node<K,V>*) nodes[5]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRLright = (Version<K>*) ((Node<K,V>*) nodes[5]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRright = (Version<K>*) ((Node<K,V>*) nodes[3]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    
    // Version<K>* nodeXXLLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 3), nodes[2]->key, verXXLLleft->sum+verXXLLright->sum, verXXLLleft, verXXLLright);
    // Version<K>* nodeXXLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), nodes[1]->key, nodeXXLLver->sum+verXXLright->sum, nodeXXLLver, verXXLright);
    // Version<K>* nodeXXRLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 4), nodes[5]->key, verXXRLleft->sum+verXXRLright->sum, verXXRLleft, verXXRLright);
    // Version<K>* nodeXXRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), nodes[3]->key, nodeXXRLver->sum+verXXRright->sum, nodeXXRLver, verXXRright);
    ...
    int weight = nodes[1]->weight;
    // Version<K>* nodeXXver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), nodes[4]->key, nodeXXLver->sum+nodeXXRver->sum, nodeXXLver, nodeXXRver);
...
template<class K, class V, class Compare, class MasterRecordMgr> bool Chromatic<K,V,Compare,MasterRecordMgr>::doW5(const int tid, Node<K,V> **nodes, void **llxResults, bool fieldIsLeft) {
    ...
    // Version<K>* verXXLLleft = (Version<K>*) ((Node<K,V>*) nodes[2]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXLLright = (Version<K>*) ((Node<K,V>*) nodes[2]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXLright = (Version<K>*) ((Node<K,V>*) nodes[3]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRleft = (Version<K>*) ((Node<K,V>*) nodes[4]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRright = (Version<K>*) ((Node<K,V>*) nodes[4]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    
    // Version<K>* nodeXXLLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 3), nodes[2]->key, verXXLLleft->sum+verXXLLright->sum, verXXLLleft, verXXLLright);
    // Version<K>* nodeXXLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), nodes[1]->key, nodeXXLLver->sum+verXXLright->sum, nodeXXLLver, verXXLright);
    // Version<K>* nodeXXRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), nodes[4]->key, verXXRleft->sum+verXXRright->sum, verXXRleft, verXXRright);
    Node<K,V> *nodeXXLL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 3), nodes[2]->key, nodes[2]->value, nodes[2]->weight-1, (Node<K,V>*) nodes[2]->left.load(memory_order_relaxed), (Node<K,V>*) nodes[2]->right.load(memory_order_relaxed));
    ...
    int weight = nodes[1]->weight;
    // Version<K>* nodeXXver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), nodes[3]->key, nodeXXLver->sum+nodeXXRver->sum, nodeXXLver, nodeXXRver);
...
template<class K, class V, class Compare, class MasterRecordMgr> bool Chromatic<K,V,Compare,MasterRecordMgr>::doW6(const int tid, Node<K,V> **nodes, void **llxResults, bool fieldIsLeft) {
    ...
    // Version<K>* verXXLLright = (Version<K>*) ((Node<K,V>*) nodes[2]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXLright = (Version<K>*) ((Node<K,V>*) nodes[4]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRleft = (Version<K>*) ((Node<K,V>*) nodes[4]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRright = (Version<K>*) ((Node<K,V>*) nodes[3]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    
    // Version<K>* nodeXXLLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 3), nodes[2]->key, verXXLLleft->sum+verXXLLright->sum, verXXLLleft, verXXLLright);
    // Version<K>* nodeXXLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), nodes[1]->key, nodeXXLLver->sum+verXXLright->sum, nodeXXLLver, verXXLright);
    // Version<K>* nodeXXRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), nodes[3]->key, verXXRleft->sum+verXXRright->sum, verXXRleft, verXXRright);
    Node<K,V> *nodeXXLL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 3), nodes[2]->key, nodes[2]->value, nodes[2]->weight-1, (Node<K,V>*) nodes[2]->left.load(memory_order_relaxed), (Node<K,V>*) nodes[2]->right.load(memory_order_relaxed));
    Node<K,V> *nodeXXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[1]->key, nodes[1]->value, 1, nodeXXLL, (Node<K,V>*) nodes[4]->left.load(memory_order_relaxed));
...
template<class K, class V, class Compare, class MasterRecordMgr> bool Chromatic<K,V,Compare,MasterRecordMgr>::doW7(const int tid, Node<K,V> **nodes, void **llxResults, bool fieldIsLeft) {
    ...
    // Version<K>* nodeXXLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), nodes[2]->key, verXXLleft->sum+verXXLright->sum, verXXLleft, verXXLright);
    // Version<K>* nodeXXRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), nodes[3]->key, verXXRleft->sum+verXXRright->sum, verXXRleft, verXXRright);
    Node<K,V> *nodeXXL = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 1), nodes[2]->key, nodes[2]->value, nodes[2]->weight-1, (Node<K,V>*) nodes[2]->left.load(memory_order_relaxed), (Node<K,V>*) nodes[2]->right.load(memory_order_relaxed));
    ...
    int weight = (IS_SENTINEL(nodes[1], nodes[0]) ? 1 : nodes[1]->weight+1); // root of old subtree is a sentinel
    // Version<K>* nodeXXver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), nodes[1]->key, nodeXXLver->sum+nodeXXRver->sum, nodeXXLver, nodeXXRver);
    Node<K,V> *nodeXX = initializeNode(tid, GET_ALLOCATED_NODE_PTR(tid, 0), nodes[1]->key, nodes[1]->value, weight, nodeXXL, nodeXXR);
...
template<class K, class V, class Compare, class MasterRecordMgr> bool Chromatic<K,V,Compare,MasterRecordMgr>::doRb1Sym(const int tid, Node<K,V> **nodes, void **llxResults, bool fieldIsLeft) {
    ...
    // Version<K>* verXLleft = (Version<K>*) ((Node<K,V>*) nodes[1]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXLright = (Version<K>*) ((Node<K,V>*) nodes[2]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXright = (Version<K>*) ((Node<K,V>*) nodes[2]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    
    // Version<K>* nodeXLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), nodes[1]->key, verXLleft->sum+verXLright->sum, verXLleft, verXLright);
    ...
    int weight = nodes[1]->weight;
    // Version<K>* nodeXver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), nodes[2]->key, nodeXLver->sum+verXright->sum, nodeXLver, verXright);
...
template<class K, class V, class Compare, class MasterRecordMgr> bool Chromatic<K,V,Compare,MasterRecordMgr>::doRb2Sym(const int tid, Node<K,V> **nodes, void **llxResults, bool fieldIsLeft) {
    ...
    // Version<K>* verXLright = (Version<K>*) ((Node<K,V>*) nodes[3]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXRleft = (Version<K>*) ((Node<K,V>*) nodes[3]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXRright = (Version<K>*) ((Node<K,V>*) nodes[2]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    
    // Version<K>* nodeXLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), nodes[1]->key, verXLleft->sum+verXLright->sum, verXLleft, verXLright);
    // Version<K>* nodeXRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), nodes[2]->key, verXRleft->sum+verXRright->sum, verXRleft, verXRright);
...
template<class K, class V, class Compare, class MasterRecordMgr> bool Chromatic<K,V,Compare,MasterRecordMgr>::doW1Sym(const int tid, Node<K,V> **nodes, void **llxResults, bool fieldIsLeft) {
    ...
    // Version<K>* verXXRLleft = (Version<K>*) ((Node<K,V>*) nodes[4]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRLright = (Version<K>*) ((Node<K,V>*) nodes[4]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRRleft = (Version<K>*) ((Node<K,V>*) nodes[3]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRRright = (Version<K>*) ((Node<K,V>*) nodes[3]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXleft = (Version<K>*) ((Node<K,V>*) nodes[2]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    
    // Version<K>* nodeXXRLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), nodes[4]->key, verXXRLleft->sum+verXXRLright->sum, verXXRLleft, verXXRLright);
    // Version<K>* nodeXXRRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 3), nodes[3]->key, verXXRRleft->sum+verXXRRright->sum, verXXRRleft, verXXRRright);
    // Version<K>* nodeXXRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), nodes[1]->key, nodeXXRLver->sum+nodeXXRRver->sum, nodeXXRLver, nodeXXRRver);
    ...
    int weight = nodes[1]->weight;
    // Version<K>* nodeXXver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), nodes[2]->key, verXXleft->sum+nodeXXRver->sum, verXXleft, nodeXXRver);
...
template<class K, class V, class Compare, class MasterRecordMgr> bool Chromatic<K,V,Compare,MasterRecordMgr>::doW3Sym(const int tid, Node<K,V> **nodes, void **llxResults, bool fieldIsLeft) {
    ...
    // Version<K>* verXXRLleft = (Version<K>*) ((Node<K,V>*) nodes[4]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRLright = (Version<K>*) ((Node<K,V>*) nodes[5]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRRRleft = (Version<K>*) ((Node<K,V>*) nodes[3]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRRRright = (Version<K>*) ((Node<K,V>*) nodes[3]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRRleft = (Version<K>*) ((Node<K,V>*) nodes[5]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXleft = (Version<K>*) ((Node<K,V>*) nodes[2]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    
    // Version<K>* nodeXXRLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), nodes[4]->key, verXXRLleft->sum+verXXRLright->sum, verXXRLleft, verXXRLright);
    // Version<K>* nodeXXRRRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 4), nodes[3]->key, verXXRRRleft->sum+verXXRRRright->sum, verXXRRRleft, verXXRRRright);
    // Version<K>* nodeXXRRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 3), nodes[1]->key, verXXRRleft->sum+nodeXXRRRver->sum, verXXRRleft, nodeXXRRRver);
    // Version<K>* nodeXXRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), nodes[5]->key, nodeXXRLver->sum+nodeXXRRver->sum, nodeXXRLver, nodeXXRRver);
...
template<class K, class V, class Compare, class MasterRecordMgr> bool Chromatic<K,V,Compare,MasterRecordMgr>::doW4Sym(const int tid, Node<K,V> **nodes, void **llxResults, bool fieldIsLeft) {
    ...
    // Version<K>* verXXLRleft = (Version<K>*) ((Node<K,V>*) nodes[5]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXLRright = (Version<K>*) ((Node<K,V>*) nodes[5]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXLleft = (Version<K>*) ((Node<K,V>*) nodes[2]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRRleft = (Version<K>*) ((Node<K,V>*) nodes[3]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRRright = (Version<K>*) ((Node<K,V>*) nodes[3]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRleft = (Version<K>*) ((Node<K,V>*) nodes[4]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    
    // Version<K>* nodeXXLRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 3), nodes[5]->key, verXXLRleft->sum+verXXLRright->sum, verXXLRleft, verXXLRright);
    // Version<K>* nodeXXLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), nodes[2]->key, verXXLleft->sum+nodeXXLRver->sum, verXXLleft, nodeXXLRver);
    // Version<K>* nodeXXRRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 4), nodes[3]->key, verXXRRleft->sum+verXXRRright->sum, verXXRRleft, verXXRRright);
    // Version<K>* nodeXXRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), nodes[1]->key, verXXRleft->sum+nodeXXRRver->sum, verXXRleft, nodeXXRRver);
...
template<class K, class V, class Compare, class MasterRecordMgr> bool Chromatic<K,V,Compare,MasterRecordMgr>::doW5Sym(const int tid, Node<K,V> **nodes, void **llxResults, bool fieldIsLeft) {
    ...
    // Version<K>* verXXLleft = (Version<K>*) ((Node<K,V>*) nodes[4]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXLright = (Version<K>*) ((Node<K,V>*) nodes[4]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRRleft = (Version<K>*) ((Node<K,V>*) nodes[3]->left.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRRright = (Version<K>*) ((Node<K,V>*) nodes[3]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    // Version<K>* verXXRleft = (Version<K>*) ((Node<K,V>*) nodes[2]->right.load(memory_order_relaxed))->version.load(memory_order_relaxed);
    
    // Version<K>* nodeXXLver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 1), nodes[4]->key, verXXLleft->sum+verXXLright->sum, verXXLleft, verXXLright);
    // Version<K>* nodeXXRRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 3), nodes[3]->key, verXXRRleft->sum+verXXRRright->sum, verXXRRleft, verXXRRright);
    // Version<K>* nodeXXRver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 2), nodes[1]->key, verXXRleft->sum+nodeXXRRver->sum, verXXRleft, nodeXXRRver);
    ...
    int weight = nodes[1]->weight;
    // Version<K>* nodeXXver = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), nodes[2]->key, nodeXXLver->sum+nodeXXRver->sum, nodeXXLver, nodeXXRver);
...
// toRetire = NULL indicates call to help set nil version pointer
template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K,V,Compare,MasterRecordMgr>::refresh(Node<K, V>* x, vector<Version<K>*>* toRetire) {
    uintptr_t oldVer = x->version.load(memory_order_relaxed); // TODO: refresh while value isn't changed?
    if(toRetire == nullptr && oldVer != (uintptr_t) nullptr) return true;
    if(oldVer == (uintptr_t) nullptr) nilCASsAttempted[tid * PREFETCH_SIZE_WORDS]++;
    uintptr_t oldVerCopy = oldVer;

    // Propagate called on empty tree (only root and root-> left)
    if (!x->left.load(memory_order_relaxed)) {
        return true;
    }
    ...
    Version<K>* leftver;
    do {
        left1 = (Node<K,V>*) x->left.load(memory_order_relaxed);
        leftver = (Version<K>*) left1->version.load(memory_order_relaxed);
        if (leftver == nullptr) {
            nilCASsAttempted[tid * PREFETCH_SIZE_WORDS]++;
            refresh(left1, toRetire);
            leftver = (Version<K>*) left1->version.load(memory_order_relaxed);
        }
    } while (left1 != (Node<K,V>*) x->left.load(memory_order_relaxed));
    Node<K, V> *right1;
    Version<K>* rightver;
    do {
        right1 = (Node<K,V>*) x->right.load(memory_order_relaxed);
        rightver = right1 ? (Version<K>*) right1->version.load(memory_order_relaxed) : NULL;
        if (rightver == nullptr && right1) {
            nilCASsAttempted[tid * PREFETCH_SIZE_WORDS]++;
            refresh(right1, toRetire);
            rightver = (Version<K>*) right1->version.load(memory_order_relaxed);
        }
    } while (right1 != (Node<K,V>*) x->right.load(memory_order_relaxed));

    // Node<K, V> *left1, *left2;
    // Version<K>* leftver = nullptr;
    // Node<K, V> *right1, *right2;
    // Version<K>* rightver = nullptr;
    // Version<K>* newVer;
    // left1 = (Node<K,V>*) x->left.load(memory_order_relaxed);
    // leftver = (Version<K>*) left1->version.load(memory_order_relaxed);
    // if(leftver == nullptr) {
    //     refresh(left1, nullptr);
    //     leftver = (Version<K>*) left1->version.load(memory_order_relaxed);
    // }
    // while(left1 != (Node<K,V>*)x->left.load()) {
    //     left1 =(Node<K,V>*) x->left.load();
    //     leftver = (Version<K>*) left1->version.load();
    //     if(leftver == nullptr) {
    //         refresh(left1, toRetire);
    //         leftver = (Version<K>*) left1->version.load();
    //     }
    ...
    if (right1) {
        // rightver = (Version<K>*) right1->version.load(memory_order_relaxed);
        // if(rightver == nullptr) {
        //     refresh(right1, nullptr);
        //     rightver = (Version<K>*) right1->version.load(memory_order_relaxed);
        // }
        // while(right1 != (Node<K,V>*)x->right.load()) {
        //     right1 = (Node<K,V>*)x->right.load();
        //     rightver = (Version<K>*) right1->version.load();
        //     if(rightver == nullptr) {
        //         refresh(right1, toRetire);
        //         rightver = (Version<K>*) right1->version.load();
        //     }
        // }
        newVer = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), x->key, leftver->sum + rightver->sum, leftver, rightver);
    } else { // root
        newVer = initializeVersion(tid, GET_ALLOCATED_VERSION_PTR(tid, 0), x->key, leftver->sum, leftver, NULL);
    }
    
    SOFTWARE_BARRIER;
    REPLACE_ALLOCATED_VERSION(tid, 0);
    bool res = x->version.compare_exchange_strong(oldVer, (uintptr_t) newVer);
    totCASsAttempted[tid * PREFETCH_SIZE_WORDS]++;
    if (res) {
        if(oldVer) toRetire->push_back((Version<K>*) oldVer);
    } else {
        recordmgr->deallocate(tid, newVer);
    }
    return res;
}

template<class K, class V, class Compare, class MasterRecordMgr>
bool Chromatic<K,V,Compare,MasterRecordMgr>::findNode(Version<K>* root, Version<K>* node) {
    if(root == node) return true;
    if(root->left.load() == 0) return false;
    Version<K>* child = cmp(node->key, root->key) || root->key == NO_KEY ? (Version<K>*) root->left.load(memory_order_relaxed) : (Version<K>*) root->right.load(memory_order_relaxed);
    return findNode(child, node);
}

template<class K, class V, class Compare, class MasterRecordMgr>
void Chromatic<K,V,Compare,MasterRecordMgr>::propagate(int tid, K key, stack<Node<K,V>*> stack) {

    vector<Version<K>*> toRetire = *(versToRetire[tid * PREFETCH_SIZE_WORDS]);
    set<Node<K,V>*> set;
    Node<K,V>* r;
    numPropagates[tid * PREFETCH_SIZE_WORDS]++;
    basePropagateSizes[tid * PREFETCH_SIZE_WORDS]+=stack.size();
    totalPropagateSizes[tid * PREFETCH_SIZE_WORDS]+=stack.size();

    while (!set.count(root)) {
        Node<K,V>* c = stack.top();
        //DEBUG COUTATOMIC("1"<<endl);
        c = cmp(key, c->key) || c->key == NO_KEY ? (Node<K,V>*) c->left.load(memory_order_relaxed) : (Node<K,V>*) c->right.load(memory_order_relaxed);
        //DEBUG COUTATOMIC("2"<<endl);
        while (c && c->left && !set.count(c)) {
            totalPropagateSizes[tid * PREFETCH_SIZE_WORDS]++;
            stack.push(c);
            c = cmp(key, c->key) || c->key == NO_KEY ? (Node<K,V>*) c->left.load(memory_order_relaxed) : (Node<K,V>*) c->right.load(memory_order_relaxed);
        }
        //DEBUG COUTATOMIC("3"<<endl);
        r = stack.top();
        stack.pop();
        // TODO: Have to make sure child doesn't change?
        numLevelsPropagated[tid * PREFETCH_SIZE_WORDS]++;
        //DEBUG COUTATOMIC("4"<<endl);
        if (!refresh(r, &toRetire)) {
            refresh(r, &toRetire);
        }
        //DEBUG COUTATOMIC("6"<<endl);
        set.insert(r);
    }
    SOFTWARE_BARRIER;
    Version<K>* verRoot = (Version<K>*) root->version.load();
    int counter = 0;
    std::string s;
    for (int j = 0; j < toRetire.size(); j++) {
        DEBUG {
            if(findNode(verRoot, toRetire[j])) {
                counter++;
                s += '1';
            }
            else{
        ...
        recordmgr->retire(tid, toRetire[j]);
    }
    DEBUG {
        if(counter) {
            std::stringstream ss;
            ss << counter << " out of " << toRetire.size() << " versions are still reachable!!" << std::endl;
            ss << s << std::endl;
            std::cerr << ss.str();
        }
    }
...
bool Chromatic<K,V,Compare,MasterRecordMgr>::recoverAnyAttemptedSCX(const int tid, const int location) {
    ...
    if (recordmgr->isQProtected(tid, myscx)) {
        ...
        Node<K,V> ** nodes = (Node<K,V> **)myscx->nodes;
    ...
    if (highestIndexReached == 0) {
        ...
        if (state == SCXRecord<K,V>::STATE_COMMITTED) {
            for (int i=0;i<NUM_INSERTED[operationType];++i) {
                REPLACE_ALLOCATED_NODE(tid, i);
                REPLACE_ALLOCATED_VERSION(tid, i);
            }
        }
        ...
        if (state == SCXRecord<K,V>::STATE_COMMITTED) {
            ...
            for (int j=1;j<nNodes;++j) {
                ...
                // recordmgr->retire(tid, (Version<K>*) nodes[j]->version.load(memory_order_relaxed));
