...
#define RECORD_MANAGER_T record_manager<Reclaim, Alloc, Pool, node_t<K,V>, version_t<K>, info_t<K,V>>
...
public:
    ...
    int getRank(const int tid, const K& key, K * const resultKeys, V * const resultValues) {
        return 1; //ds->getRank(tid, key);
    }
    K selectItem(const int tid, int rank) {
        setbench_error("not implemented");
    }
    int rangeQuery(const int tid, const K& lo, const K& hi, K * const resultKeys, V * const resultValues) {
        return ds->bst_rangequery(tid, lo, hi);
    }
    void printSummary() {
        //ds->printTree();
