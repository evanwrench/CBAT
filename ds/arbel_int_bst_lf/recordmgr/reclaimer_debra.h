...
class reclaimer_debra_bundle : public BUNDLE_INTERFACE::reclaimer_interface<T, Pool> {
protected:
...
public:
    ...
    struct rebind {
        typedef reclaimer_debra_bundle<_Tp1, Pool> other;
    };
    template<typename _Tp1, typename _Tp2>
    struct rebind2 {
        typedef reclaimer_debra_bundle<_Tp1, _Tp2> other;
    };
    ...
    inline bool leaveQuiescentState(const int tid, void * const * const reclaimers, const int numReclaimers, const bool readOnly = false) {
        ...
        if (readEpoch != ann /* invariant: ann is not quiescent */) {
            ...
            for (int i=0;i<numReclaimers;++i) {
                ((reclaimer_debra_bundle<T, Pool> * const) reclaimers[i])->rotateEpochBags(tid);
            }
            result = true;
        }
...
//        }
    ...
    reclaimer_debra_bundle(const int numProcesses, Pool *_pool, debugInfo * const _debug, RecMgr::RecoveryMgr<void *> * const _recoveryMgr = NULL)
            : BUNDLE_INTERFACE::reclaimer_interface<T, Pool>(numProcesses, _pool, _debug, _recoveryMgr) {
        VERBOSE cout<<"constructor reclaimer_debra_bundle helping="<<this->shouldHelp()<<endl;// scanThreshold="<<scanThreshold<<endl;
        epoch = 0;
    ...
    ~reclaimer_debra_bundle() {
        VERBOSE DEBUG cout<<"destructor reclaimer_debra_bundle"<<endl;
        for (int tid=0;tid<this->NUM_PROCESSES;++tid) {
            for (int i=0;i<NUMBER_OF_EPOCH_BAGS;++i) {
