...
#endif
#include "./bundle_citrus/bundle_citrus_impl.h"
#include "./rq/rq_bundle.h"
#include "./recordmgr/reclaimer_debra.h"

// Only debra reclamation available for this ds
#define RECORD_MANAGER_T ArbelMgr::record_manager<reclaimer_debra_bundle<K>, Alloc, Pool, node_t<K,V>>
#define DATA_STRUCTURE_T bundle_citrustree<K,V,RECORD_MANAGER_T>

template <typename K, typename V, class Reclaim = reclaimer_debra_bundle<K>, class Alloc = allocator_new<K>, class Pool = pool_none<K>>
class ds_adapter {
private:
    const V NO_VALUE;
...
public:
    ...
    , ds(new DATA_STRUCTURE_T(KEY_MAX, NO_VALUE, NUM_THREADS))
    {}
    ~ds_adapter() {
        delete ds;
    }
